...
class HVDCContextProtocol:
    ...
    async def create_context_for_command(self, command: str, parameters: Dict[str, Any]) -> HVDCContextWindow:
        ...
        if command == "enhance_dashboard":
            context.prompt = f"대시보드 강화: {parameters.get('enhancement_type', 'general')}"
            context.examples = [
                "weather_integration: 날씨 데이터 통합",
                "ocr_processing: OCR 처리 기능",
                "kpi_monitoring: KPI 모니터링"
            ]
            context.tools = ["dashboard_api", "weather_api", "ocr_engine", "kpi_calculator"]
            context.state = {"mode": "LATTICE", "dashboard_id": parameters.get("dashboard_id", "main")}
            ...
            context.prompt = f"기상 조건 분석: {parameters.get('weather_data', 'general')}"
            context.examples = [
                "storm: 폭풍 조건 분석",
                "delay: 지연 시간 계산",
                "eta_update: ETA 업데이트"
            ...
            context.state = {"mode": "RHYTHM", "weather_condition": parameters.get("weather_data")}
    ...
    async def update_context_with_response(self, context: HVDCContextWindow, response: Dict[str, Any]) -> None:
        """응답으로 Context 업데이트"""
        # Memory에 응답 추가
        memory_entry = {
...
class HVDCContextEngineeringIntegration:
    ...
    async def execute_command_with_context(self, command: str, parameters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 명령어 실행
            result = await self.logi_master_system.execute_command(command, parameters or {})
    ...
    async def get_context_analytics(self) -> Dict[str, Any]:
        ...
        # 품질 분포
        context_quality_distribution = self._calculate_quality_distribution(context_scores)
        response_quality_distribution = self._calculate_quality_distribution(response_scores)
        
        # 가장 많이 사용된 도구
        most_used_tools = sorted(tools_usage.items(), key=lambda x: x[1], reverse=True)[:5]
        
        return {
            "total_contexts": total_contexts,
            "average_context_score": sum(context_scores) / len(context_scores) if context_scores else 0.0,
            "average_response_score": sum(response_scores) / len(response_scores) if response_scores else 0.0,
