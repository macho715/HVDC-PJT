...
class LogiMasterNavigation:
    ...
    def __init__(self):
        ...
        self.dashboard_hierarchy = {
            ...
            "hvdc_dashboard_main.html": {"parent": "index.html", "name": "HVDC Dashboard Main", "level": 1},
            "hvdc_warehouse_monitor.html": {"parent": "index.html", "name": "Warehouse Monitor", "level": 1},
            "hvdc_inventory_tracking.html": {"parent": "index.html", "name": "Inventory Tracking", "level": 1},
            "macho_realtime_kpi_dashboard.py": {"parent": "index.html", "name": "MACHO KPI Dashboard", "level": 1},
            "tdd_progress_dashboard.html": {"parent": "index.html", "name": "TDD Progress Dashboard", "level": 1},
            "logi_master_enhanced_dashboard.html": {"parent": "index.html", "name": "LOGI MASTER Enhanced", "level": 1},
            "logi_master_main_dashboard.html": {"parent": None, "name": "LOGI MASTER Main Dashboard", "level": 0}
        }
    
    def create_navigation_header(self, current_file: str) -> str:
        """네비게이션 헤더 HTML 생성"""
        current_info = self.dashboard_hierarchy.get(current_file, {"parent": None, "name": "Unknown", "level": 0})
    ...
    def _generate_breadcrumb(self, current_file: str) -> str:
        ...
        while current and current in self.dashboard_hierarchy:
            ...
            if info["name"] != "LOGI MASTER Main Dashboard":  # 메인 대시보드는 제외
                breadcrumb.append(f'<a href="{current}" style="color: #3498db; text-decoration: none;">{info["name"]}</a>')
            current = info["parent"]
    ...
    def add_navigation_to_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if match:
                # head 태그 뒤에 네비게이션 추가
                new_content = content[:match.end()] + navigation_html + content[match.end():]
            else:
    ...
    def add_floating_navigation_to_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if match:
                new_content = content[:match.start()] + floating_nav + content[match.start():]
            else:
