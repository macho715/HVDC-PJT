        ...
        for _, row in df.iterrows():
            ...
            if has_warehouse_date and has_site_date:
                warehouse_outbound_items.append(
                    {"site": site_name, "date": site_date, "case_no": row["Case No."]}
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.exists(data_file):
            ...
            print(
                f"✅ 데이터 로드 완료: {len(hvdc_data)} 행, {len(hvdc_data.columns)} 컬럼"
            )
            ...
            print(
                f"   - 재고 = 입고 - 출고: {inventory_result['inbound_total']} - {inventory_result['outbound_total']} = {inventory_result['current_inventory']}"
            )
            ...
            if inventory_result["outbound_total"] > inventory_result["inbound_total"]:
                print(
                    "❌ 경고: 출고가 입고보다 많습니다! 이는 논리적으로 불가능합니다."
                )
            ...
            print(f"\n📊 상세 분석:")
            print(
                f"   - 입고율: {(inventory_result['inbound_total'] / len(hvdc_data) * 100):.1f}%"
            )
            print(
                f"   - 출고율: {(inventory_result['outbound_total'] / len(hvdc_data) * 100):.1f}%"
            )
            print(
                f"   - 재고율: {(inventory_result['current_inventory'] / len(hvdc_data) * 100):.1f}%"
            )
