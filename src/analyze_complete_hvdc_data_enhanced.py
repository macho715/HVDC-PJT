...
class EnhancedHVDCAnalyzer:
    ...
    def identify_columns(self):
        ...
        for col in self.df.columns:
            ...
            # 창고 컬럼 식별
            if any(wh in col_lower for wh in ['warehouse', 'wh', 'storage', 'dsv', 'aaa']):
            ...
            # 날짜 컬럼 식별
            if any(date_word in col_lower for date_word in ['date', 'arrival', 'inbound', 'outbound']):
                ...
                for key, value in self.analysis_results.items():
                    ...
                    if isinstance(value, dict):
                        for sub_key, sub_value in value.items():
                            if sub_key not in ['inbound_items', 'site_items', 'direct_items', 'warehouse_items']:
                                summary_data.append({
