        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    result['data']['operation_risk'] = self._get_risk_level(impact_score)
                    result['data']['recommendations'] = self._get_weather_recommendations(impact_score)
                    
                    logger.info(f"🌤️ 실제 날씨 데이터 수집 완료: {port_code} - {result['data']['weather_condition']}")
                    return result
                else:
    ...
    async def get_real_ocr_data(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(file_path, 'rb') as file:
                ...
                async with self.session.post(config.base_url, data=data, files=files) as response:
                    if response.status == 200:
                        ...
                        result = {
                            ...
                            'data': {
                                ...
                                'processing_time': ocr_result.get('ProcessingTimeInMilliseconds', 0) / 1000,
                                'extracted_data': self._extract_invoice_data(parsed_text),
                                'ocr_confidence': parsed_results[0].get('TextOverlay', {}).get('Lines', [{}])[0].get('Confidence', 0)
                            },
                            'timestamp': datetime.now().isoformat()
                        }
                        
                        logger.info(f"📄 실제 OCR 처리 완료: {file_path} - {result['data']['word_count']}단어")
                        return result
                    else:
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    async with self.session.get(position_url, params=position_params) as pos_response:
                        position_data = await pos_response.json() if pos_response.status == 200 else {}
                    ...
                    if result['data']['eta'] and result['data']['eta'] != 'Unknown':
                        ...
                        if eta_datetime:
                            ...
                            result['data']['eta_delay'] = self._calculate_eta_delay(eta_datetime)
                    
                    logger.info(f"🚢 실제 선박 데이터 수집 완료: {vessel_id} - {result['data']['vessel_name']}")
                    return result
                else:
                ...
                if response.status == 200:
                    ...
                    result = {
                        ...
                        'data': {
                            ...
                            'berth_availability': port_data.get('berth_availability', []),
...
async def main():
    ...
    try:
        ...
        print(f"🚢 선박 데이터: {shipping_result['status']} - {shipping_result['source']}")
