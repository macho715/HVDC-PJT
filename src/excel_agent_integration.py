...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "..", "..", "python_excel_agent-main")
)
try:
...
class ExcelAgentIntegration:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.hvdc_config = {
            ...
            "status_columns": [
                col
                for col in ["Status_Current", "Status_Location", "Status_WAREHOUSE"]
                if col
            ],
        }
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if hvdc_data_path.exists():
                ...
                self.logger.info(
                    f"Default HVDC data loaded: {len(self.current_dataframe)} rows"
                )
    ...
    async def load_excel_file(
        self, file_path: str, dataframe_name: str = None
    ) -> Dict[str, Any]:
        """Excel 파일 로드"""
        try:
            ...
            self.logger.info(
                f"Excel file loaded: {file_path} -> {len(df)} rows, {len(df.columns)} columns"
            )
            return result
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "total_rows": len(df) if df is not None else 0}
    ...
    def _validate_hvdc_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # 현장 컬럼 확인
            site_found = [
                col for col in self.hvdc_config["site_columns"] if col in df.columns
            ]
    ...
    async def process_natural_language_query(
        self, query: str, dataframe_name: str = None
    ) -> Dict[str, Any]:
        """자연어 쿼리 처리"""
        try:
            ...
            self.logger.info(
                f"Query processed: {query} -> Confidence: {combined_result['confidence']}"
            )
    ...
    async def _process_hvdc_specific_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 상태 분석
                ...
                return {"status_analysis": status_analysis}
    ...
    async def get_dataframe_info(self, dataframe_name: str = None) -> Dict[str, Any]:
        ...
        try:
            df = self.current_dataframe
            if df is None:
                return {"status": "ERROR", "error": "No data loaded", "confidence": 0.0}
    ...
    async def export_analysis_report(self, output_path: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # HVDC 특화 분석
            hvdc_analysis = await self._process_hvdc_specific_query(
                "창고 현장 HVDC 코드 상태 분석"
            )
...
class ExcelAgentMACHOAdapter:
    ...
    async def execute_command(
        ...
        try:
            ...
            if command == "load_excel":
                ...
                dataframe_name = parameters.get("dataframe_name")
                return await self.excel_agent.process_natural_language_query(
                    query, dataframe_name
                )
    ...
    def get_available_commands(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"name": "get_status", "description": "시스템 상태 조회", "parameters": []},
        ]
