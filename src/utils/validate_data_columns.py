...
def validate_vendor_data_quality(
    hitachi_file: str, simense_file: str
) -> Dict[str, Any]:
    """
    벤더별 데이터 품질 종합 검증
    ...
    # 3. 통합 데이터 검증
    if (
        "error" not in validation_results["hitachi_stats"]
        and "error" not in validation_results["simense_stats"]
    ):
        ...
        for col in df_combined.columns:
            if col in ["Case No.", "Vendor", "Status_Current", "Status_Location"]:
                ...
                loc in col for loc in ["DSV", "AAA", "MOSB", "AGI", "DAS", "MIR", "SHU"]
            ):
                print(f"  📍 위치 컬럼: {col}")
        ...
        print("\n📋 상태별 데이터 분석:")
        status_columns = [
            col for col in df_combined.columns if "Status" in col or "Current" in col
        ]
        ...
        actual_hitachi = validation_results["combined_stats"][
            "vendor_distribution"
        ].get("HITACHI", 0)
        actual_simense = validation_results["combined_stats"][
            "vendor_distribution"
        ].get("SIMENSE", 0)
        ...
        if actual_hitachi > target_hitachi:
            recommendations.append(
                f"HITACHI 데이터 {actual_hitachi - target_hitachi}건 초과 - 상태별 필터링 필요"
            )
        if actual_simense > target_simense:
            recommendations.append(
                f"SIMENSE 데이터 {actual_simense - target_simense}건 초과 - 상태별 필터링 필요"
            )
        if actual_total > target_total:
            recommendations.append(
                f"총 데이터 {actual_total - target_total}건 초과 - 완료된 아이템 제거 필요"
            )
        ...
        print(f"\n📈 목표값 대비 현황:")
        print(
            f"  HITACHI: {actual_hitachi} vs {target_hitachi} ({actual_hitachi - target_hitachi:+d})"
        )
        print(
            f"  SIMENSE: {actual_simense} vs {target_simense} ({actual_simense - target_simense:+d})"
        )
        print(
            f"  Total: {actual_total} vs {target_total} ({actual_total - target_total:+d})"
        )
...
def analyze_status_filtering(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    status_columns = [
        ...
        if "Status" in col or "Current" in col or "Complete" in col
    ]
            ...
            try:
                ...
                if not pd.api.types.is_datetime64_any_dtype(df[col]):
                    status_vendor_counts = (
                        df.groupby([col, "Vendor"]).size().unstack(fill_value=0)
                    )
                    print(f"    벤더별 분포:\n{status_vendor_counts}")
...
def apply_status_filtering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Status_Storage" in df_filtered.columns:
        ...
        if storage_pre_arrival_count > 0:
            ...
            print(
                f"  ✅ Status_Storage Pre Arrival 제거: {storage_pre_arrival_count}건"
            )
    ...
    if "Status_Location" in df_filtered.columns:
        ...
        if location_pre_arrival_count > 0:
            ...
            print(
                f"  ✅ Status_Location Pre Arrival 제거: {location_pre_arrival_count}건"
            )
    ...
    print(
        f"  📊 필터링 결과: {original_count} → {filtered_count} (제거: {removed_count}건)"
    )
...
def validate_filtered_data_quality(df_filtered: pd.DataFrame) -> Dict[str, Any]:
    ...
    validation_results["target_comparison"] = {
        "hitachi": {
            "actual": actual_hitachi,
            "target": target_hitachi,
            "difference": actual_hitachi - target_hitachi,
        },
        "simense": {
            "actual": actual_simense,
            "target": target_simense,
            "difference": actual_simense - target_simense,
        },
        "total": {
            "actual": actual_total,
            "target": target_total,
            "difference": actual_total - target_total,
        },
    }
    ...
    if actual_hitachi > target_hitachi:
        validation_results["recommendations"].append(
            f"HITACHI 여전히 {actual_hitachi - target_hitachi}건 초과"
        )
    if actual_simense > target_simense:
        validation_results["recommendations"].append(
            f"SIMENSE 여전히 {actual_simense - target_simense}건 초과"
        )
    if actual_total > target_total:
        validation_results["recommendations"].append(
            f"총 데이터 여전히 {actual_total - target_total}건 초과"
        )
...
def validate_hitachi_location_targets(df_filtered: pd.DataFrame) -> None:
    ...
    for loc, target in hitachi_targets.items():
        ...
        total_target += target
    print(
        f"  {'TOTAL':<15} 목표: {total_target:<4} | 실제: {total_actual:<4} {'✅' if total_actual == total_target else f'❌ (차이: {total_actual - total_target:+d})'}"
    )
...
if __name__ == "__main__":
    ...
    pivot = summarize_location_pivot(
        ...
        sublocation_col=(
            "Status_Location_DSV Indoor"
            if "Status_Location_DSV Indoor" in df_hitachi.columns
            else None
        ),
    )
    print(pivot)
    ...
    if columns_col:
        ...
        print(
            "⚠️ 'total handling' 컬럼이 데이터에 없습니다. 열 필드 없이 피벗을 생성합니다."
        )
