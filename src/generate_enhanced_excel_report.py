...
class EnhancedExcelReporter:
    ...
    def standardize_hitachi_data(self, df):
        ...
        # Flow Code 계산
        standardized["Flow_Code"] = self.calculate_flow_code(
            standardized, warehouse_cols
        )
    ...
    def generate_transaction_summary_sheet(self, df):
        ...
        hitachi_pct = (
            (hitachi_count / total_transactions * 100) if total_transactions > 0 else 0
        )
        simense_pct = (
            (simense_count / total_transactions * 100) if total_transactions > 0 else 0
        )
    ...
    def generate_warehouse_monthly_io_sheet(self, df):
        ...
        while current_date <= end_date:
            ...
            for warehouse in self.calc.warehouse_columns:
                ...
                if "by_warehouse_month" in month_inbound_all:
                    inbound_count = (
                        month_inbound_all["by_warehouse_month"]
                        .get(warehouse, {})
                        .get(month_str, 0)
                    )
                else:
                ...
                # 전체 출고를 창고별로 분배 (임시)
                row_data[outbound_key] = (
                    outbound_count // len(self.calc.warehouse_columns)
                    if outbound_count > 0
                    else 0
                )
        ...
        print(
            f"   ✅ 창고_월별_입출고 생성 완료: {len(monthly_data)}행 × {len(monthly_df.columns)}열"
        )
        return monthly_df
    ...
    def generate_site_monthly_inventory_sheet(self, df):
        ...
        while current_date <= end_date:
            ...
            for site in self.calc.site_columns:
                ...
                # 전체 입고를 현장별로 분배 (임시)
                row_data[inbound_key] = (
                    inbound_count // len(self.calc.site_columns)
                    if inbound_count > 0
                    else 0
                )
                ...
                while temp_date <= current_date:
                    ...
                    cumulative_inbound += (
                        temp_inbound // len(self.calc.site_columns)
                        if temp_inbound > 0
                        else 0
                    )
        ...
        print(
            f"   ✅ 현장_월별_입고재고 생성 완료: {len(monthly_data)}행 × {len(monthly_df.columns)}열"
        )
        return monthly_df
    ...
    def generate_excel_report(self, output_path=None):
        ...
        warehouse_monthly_sheet = self.create_multi_level_headers(
            warehouse_monthly_sheet, "warehouse"
        )
        ...
        try:
            with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
                ...
                # 시트 2: 창고_월별_입출고
                warehouse_monthly_sheet.to_excel(
                    writer, sheet_name="창고_월별_입출고", index=False
                )

                # 시트 3: 현장_월별_입고재고
                site_monthly_sheet.to_excel(
