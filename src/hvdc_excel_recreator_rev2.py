...
class FlowCodeCalculatorV2:
    ...
    def calc(self, df: pd.DataFrame) -> pd.Series:
        ...
        warehouse_mask = (df['Status_Storage'].str.lower() == 'warehouse') & (~pre_arrival_mask)
        flow_codes[warehouse_mask] = 1
        
        # Rule 2: MOSB 경유 (Status_Location에 MOSB 포함)
        mosb_mask = (df['Status_Location'].str.contains('MOSB', na=False, case=False)) & (~pre_arrival_mask)
        flow_codes[mosb_mask] = 2
        
        # Rule 3: Warehouse에서 Site로 이동 (창고 위치에 있는 경우)
        warehouse_locations = ['DSV Indoor', 'DSV Outdoor', 'DSV Al Markaz', 'AAA  Storage', 'Hauler Indoor', 'DSV MZP']
        wh_location_mask = (df['Status_Location'].isin(warehouse_locations)) & (~pre_arrival_mask) & (~mosb_mask)
        flow_codes[wh_location_mask] = 3
        
        # Rule 4: Site 최종 위치 (Status_Storage == 'site' 또는 실제 현장 위치)
        site_locations = ['SHU', 'DAS', 'MIR', 'AGI']
        site_mask = ((df['Status_Storage'].str.lower() == 'site') | 
                    (df['Status_Location'].isin(site_locations))) & (~pre_arrival_mask) & (~mosb_mask) & (~wh_location_mask)
...
class HVDCExcelRecreatorREV2:
    ...
    def standardize_case_list(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_std.select_dtypes(include=['object']).columns:
            df_std[col] = df_std[col].astype(str).str.replace('\u3000', ' ')  # full-width space
    ...
    def calculate_stock_levels(self, inbound_df: pd.DataFrame, outbound_df: pd.DataFrame) -> pd.DataFrame:
        """Calculate cumulative stock = inbound cum - outbound cum"""
        # Align dataframes
        inbound_filled = inbound_df.reindex(inbound_df.index.union(outbound_df.index), fill_value=0)
        outbound_filled = outbound_df.reindex(inbound_df.index.union(outbound_df.index), fill_value=0)
    ...
    def make_warehouse_io_table(self):
        ...
        simense_df = pd.read_excel('../data/HVDC WAREHOUSE_SIMENSE(SIM).xlsx', sheet_name='Case List')
        ...
        # 창고 리스트 정의
        warehouses = ['DSV Indoor', 'DSV Outdoor', 'DSV Al Markaz', 'DSV MZP', 'AAA Storage', 'Hauler Indoor', 'MOSB']
        ...
        # 창고별 특성 정의
        warehouse_info = {
            'DSV Indoor': {'capacity': 2000, 'utilization': 75.2, 'type': 'Indoor', 'share': 0.28},
            'DSV Outdoor': {'capacity': 5000, 'utilization': 68.5, 'type': 'Outdoor', 'share': 0.32},
            'DSV Al Markaz': {'capacity': 3000, 'utilization': 82.1, 'type': 'Central', 'share': 0.25},
            'MOSB': {'capacity': 1500, 'utilization': 45.8, 'type': 'Offshore', 'share': 0.15}
        }
        
        # 실제 창고별 입출고 계산
        for warehouse_name in warehouses:
            self._calculate_warehouse_io_correct(wh_io_table, warehouse_name, df, warehouse_info, total_warehouse_flow)
        
        return wh_io_table
    
    def _calculate_warehouse_io_correct(self, wh_io_table, warehouse_name, df, warehouse_info, total_warehouse_flow):
        ...
        if actual_col and actual_col in df.columns:
            ...
            warehouse_dates = pd.to_datetime(df[df[actual_col].notna()][actual_col], errors='coerce').dropna()
            ...
            if len(warehouse_dates) > 0:
                ...
                for month_period, inbound_count in monthly_inbound.items():
                    ...
                    if month_str in wh_io_table.index:
                        ...
                        wh_io_table.loc[month_str, ('출고', warehouse_name)] = outbound_count
        ...
        print(f"   ✓ 출고율: {(total_outbound/total_inbound*100):.1f}%" if total_inbound > 0 else "   ✓ 출고율: 0%")
    ...
    def make_site_stock_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Status_Location_Date' in df.columns:
            status_dates = pd.to_datetime(df['Status_Location_Date'], errors='coerce').dropna()
        ...
        multi_columns = pd.MultiIndex.from_arrays([level1, level2], names=['구분', 'Site'])
        
        # DataFrame 생성
        site_table = pd.DataFrame(
            index=date_range.strftime('%Y-%m'),
        ...
        for site_name, date_col in site_date_cols.items():
            ...
            if 'Site' in df.columns and 'Status_Location_Date' in df.columns:
                ...
                site_status_dates = pd.to_datetime(
                    df.loc[site_mask, 'Status_Location_Date'], 
                    errors='coerce'
                ).dropna()
                
                if len(site_status_dates) > 0:
                    monthly_inbound_status = site_status_dates.dt.to_period('M').value_counts()
                    
                    for month, count in monthly_inbound_status.items():
                        month_str = str(month)
                        if month_str in site_table.index:
                            # 기존 값과 합산 (중복 제거를 위해 max 사용)
                            existing_val = site_table.loc[month_str, ('입고', site_name)]
                            if pd.isna(existing_val):
                                existing_val = 0
                            site_table.loc[month_str, ('입고', site_name)] = max(existing_val, count)
    ...
    def create_5_sheet_report(self, output_file: str) -> None:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            all_cases.to_excel(writer, sheet_name='전체_트랜잭션_FLOWCODE0-4', index=False)
            
            # Sheet 2: FLOWCODE0-4_분석요약
            flow_summary = all_cases.groupby('FLOW_CODE').size().reset_index(name='Count')
            flow_summary.to_excel(writer, sheet_name='FLOWCODE0-4_분석요약', index=False)
    ...
    def _apply_multi_header_formatting(self, sheet):
        ...
        dsv_indoor_outdoor_fill = PatternFill(start_color='FF6B6B', end_color='FF6B6B', fill_type='solid')  # Red
        dsv_al_markaz_fill = PatternFill(start_color='4ECDC4', end_color='4ECDC4', fill_type='solid')  # Blue/Teal
    ...
    def assert_suite_rev2(self, df: pd.DataFrame) -> None:
        ...
        assert pre_arrival_count > 0, f"Pre-Arrival should have some records, got {pre_arrival_count}"
        ...
        assert wh_io.columns.nlevels == 2, f"Warehouse table should have 2-level headers, got {wh_io.columns.nlevels}"
        assert site_stock.columns.nlevels == 2, f"Site table should have 2-level headers, got {site_stock.columns.nlevels}"
