...
class WarehouseCalculator:
    ...
    def process_real_data(self, df: pd.DataFrame):
        ...
        date_columns = ['ETD/ATD', 'ETA/ATA', 'Status_Location_Date'] + self.warehouse_columns + self.site_columns
    ...
    def _override_flow_code(self, df: pd.DataFrame):
        ...
        if status_col in df.columns:
            is_pre_arrival = df[status_col].str.contains('Pre Arrival', case=False, na=False)
        else:
            ...
            for location in all_locations:
                if location in row.index and pd.notna(row[location]):
                    try:
                        ...
                        is_transfer_destination = any(t['to_warehouse'] == location for t in transfers)
                        
                        if not is_transfer_destination:
                            inbound_items.append({
                                'Item_ID': idx, 'Location': location, 'Warehouse': location,
                                'Inbound_Date': arrival_date, 'Year_Month': arrival_date.strftime('%Y-%m'),
                                'Vendor': row.get('Vendor', 'Unknown'), 'Pkg_Quantity': pkg_quantity,
                                'Status_Location': row.get('Status_Location', 'Unknown'),
                                'Inbound_Type': 'external_arrival'
                            })
                            total_inbound += pkg_quantity
                            by_warehouse[location] = by_warehouse.get(location, 0) + pkg_quantity
                            month_key = arrival_date.strftime('%Y-%m')
                            by_month[month_key] = by_month.get(month_key, 0) + pkg_quantity
                    except Exception as e:
                        logger.warning(f"Error parsing date (Row {idx}, Location {location}): {e}")
                        continue
        
        logger.info("✅ Inbound calculation complete.")
        return {
            'total_inbound': total_inbound, 'by_warehouse': by_warehouse, 'by_month': by_month,
            'inbound_items': inbound_items, 'warehouse_transfers': warehouse_transfers
        }
        ...
        for idx, row in df.iterrows():
            ...
            for transfer in transfers:
                ...
                outbound_items.append({
                    'Item_ID': idx, 'From_Location': from_wh, 'To_Location': transfer['to_warehouse'],
                    'Warehouse': from_wh, 'Outbound_Date': transfer_date,
                    'Year_Month': transfer_date.strftime('%Y-%m'), 'Pkg_Quantity': pkg_quantity,
                    'Status_Location': row.get('Status_Location', 'Unknown'), 'Outbound_Type': 'warehouse_transfer'
                })
                ...
                if location in row.index and pd.notna(row[location]):
                    try:
                        ...
                        if next_movements:
                            ...
                            outbound_items.append({
                                'Item_ID': idx, 'From_Location': location, 'To_Location': next_location,
                                'Warehouse': location, 'Site': next_location, 'Outbound_Date': next_date,
                                'Year_Month': next_date.strftime('%Y-%m'), 'Pkg_Quantity': pkg_quantity,
                                'Status_Location': row.get('Status_Location', 'Unknown'),
                        ...
                        logger.warning(f"Error in outbound calculation (Row {idx}, Location {location}): {e}")
                        continue
        
        logger.info("✅ Outbound calculation complete.")
        return {
            'total_outbound': total_outbound, 'by_warehouse': by_warehouse,
            'by_month': by_month, 'outbound_items': outbound_items
        }
                    ...
                    for idx, row in at_location.iterrows():
                        if location in row.index and pd.notna(row[location]):
                            try:
                                ...
                                logger.warning(f"Error in inventory calculation (Row {idx}, Location {location}): {e}")
                                continue
                    inventory_by_month[month_str][location] = inventory_count
                    inventory_by_location[location] = inventory_by_location.get(location, 0) + inventory_count
        
        total_inventory = sum(inventory_by_location.values())
        logger.info("✅ Inventory calculation complete.")
        return {
            'inventory_by_month': inventory_by_month, 'inventory_by_location': inventory_by_location,
            'total_inventory': total_inventory,
            'status_location_distribution': df['Status_Location'].value_counts().to_dict() if 'Status_Location' in df.columns else {}
        }
            ...
            for site in self.site_columns:
                if site in row.index and pd.notna(row[site]):
                    try:
                        ...
                        direct_items.append({
                            'Item_ID': idx, 'Site': site, 'Direct_Date': site_date,
                            'Year_Month': site_date.strftime('%Y-%m'), 'Flow_Code': row['FLOW_CODE'],
                            'Pkg_Quantity': utils.get_pkg(row)
                        })
                    except:
                        continue
    ...
    def calculate_cumulative_sqm_inventory(self, inbound_sqm: Dict, outbound_sqm: Dict) -> Dict:
        ...
        for month_str in month_strings:
            ...
            for warehouse in self.warehouse_columns:
                ...
                current_inventory[warehouse] = max(0, current_inventory[warehouse] + net_change)
                base_capacity = self.warehouse_base_sqm.get(warehouse, 1)
                
                cumulative_inventory[month_str][warehouse] = {
                    'inbound_sqm': monthly_inbound, 'outbound_sqm': monthly_outbound,
                    'net_change_sqm': net_change, 'cumulative_inventory_sqm': current_inventory[warehouse],
                    'utilization_rate_%': (current_inventory[warehouse] / base_capacity) * 100,
            ...
            for warehouse in self.warehouse_columns:
                if warehouse in month_data:
                    ...
                    monthly_charges[month_str][warehouse] = {
                        'sqm_used': sqm_used, 'sqm_rate_aed': sqm_rate,
    ...
    def analyze_sqm_data_quality(self, df: pd.DataFrame) -> Dict:
        ...
        for _, row in df.iterrows():
            ...
            if source_type == 'ACTUAL':
                ...
                actual_sqm_sources[source_column] = actual_sqm_sources.get(source_column, 0) + 1
            else:
