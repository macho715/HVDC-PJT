...
class TestMonthlyBalance:
    ...
    def complex_test_data(self):
        ...
        return pd.DataFrame({
            ...
            'DSV Outdoor': [datetime(2024, 1, 20), datetime(2024, 1, 25), None, None, None, None],
            ...
            'MIR': [datetime(2024, 1, 25), None, None, None, None, datetime(2024, 1, 10)],  # M001: 창고 경유, M006: 직송
            'SHU': [None, datetime(2024, 1, 30), None, None, None, None],  # M002: 창고 경유
            'DAS': [None, None, datetime(2024, 2, 5), None, None, None],   # M003: 창고 경유
            'AGI': [None, None, None, datetime(2024, 2, 10), None, None],  # M004: 창고 경유
    ...
    def test_monthly_balance_validation_should_pass_with_correct_data(self, calc, complex_test_data):
        """
        ...
        monthly_warehouse_transfer = calc.calculate_monthly_warehouse_transfer(complex_test_data)
        ...
        for month in monthly_outbound.keys():
            ...
            # 디버깅 정보 출력
            print(f"월별 Balance 검증 - {month}: 출고({outbound_count}), 현장입고({site_inbound_count}), 창고이전({warehouse_transfer_count})")
            
            # 수정된 검증: 출고 ≤ 현장 입고 (직송 포함하므로 현장 입고가 더 클 수 있음)
            assert outbound_count <= site_inbound_count, \
                f"월별 Balance 불일치 - {month}: 출고({outbound_count}) > 현장입고({site_inbound_count})"
    
    def test_outbound_event_deduplication_should_prevent_double_counting(self, calc, complex_test_data):
        """
        출고 이벤트 중복 제거 검증
        ...
        assert 'M001' not in outbound_result.get('duplicated_items', []), \
            "M001 아이템이 중복 출고로 집계되면 안됨"
        
        # 전체 출고 건수 검증
        total_outbound = outbound_result['total_outbound']
        expected_max_outbound = 6  # 최대 6개 아이템의 출고 이벤트
        assert total_outbound <= expected_max_outbound, \
            f"출고 이벤트 중복 발생 - 총 출고({total_outbound}) > 최대 예상({expected_max_outbound})"
    
    def test_direct_delivery_integration_should_include_in_site_inbound(self, calc, complex_test_data):
        """
        ...
        if len(direct_items) > 0:
            ...
            assert 'M006' in direct_item_ids, f"M006이 직송 아이템으로 인식되어야 함. 실제 직송 아이템: {direct_item_ids}"
        else:
        ...
        assert total_direct <= total_site_inbound, \
            f"직송({total_direct})이 현장 입고({total_site_inbound})에 포함되지 않음"
    
    def test_inventory_without_consumption_assumption_should_use_real_data(self, calc, complex_test_data):
        """
        ...
        warehouse_items = complex_test_data[complex_test_data['Status_Current'] == 'warehouse']
    ...
    def test_global_variable_elimination_should_use_local_calculations(self, calc, complex_test_data):
        """
        ...
        assert 'global_variable_error' not in outbound_result, \
            "전역 변수 inbound_data 참조 오류 발생"
        
        # 출고 건수가 0이 아니어야 함 (전역 변수 오류 시 0으로 처리됨)
        assert outbound_result['total_outbound'] > 0, \
            "출고 건수가 0 - 전역 변수 오류로 인한 잘못된 계산 의심"
    
    def test_kpi_outbound_inbound_accuracy_should_be_above_99_percent(self, calc, complex_test_data):
        """
        KPI: 출고-입고 일치율 ≥ 99% 검증
    ...
    def test_fail_safe_mode_trigger_should_activate_on_low_accuracy(self, calc, complex_test_data):
        """
        Fail-safe 모드 트리거 검증
