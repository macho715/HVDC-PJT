...
class MachoGPTRealIntegration:
    ...
    async def get_real_shipping_data(self, vessel_id: str) -> RealMachoGPTResult:
        ...
        try:
            # 실제 선박 API 호출
            shipping_result = await self.api_integration.get_real_shipping_data(vessel_id)
    ...
    async def execute_real_heat_stow_analysis(self, container_data: List[Dict[str, Any]]) -> RealMachoGPTResult:
        """실제 Heat-Stow 분석 (실제 센서 데이터 기반)"""
        try:
            # 실제 컨테이너 데이터 분석
            total_weight = sum(container.get('weight', 0) for container in container_data)
            total_volume = sum(container.get('volume', 0) for container in container_data)
            ...
            temperatures = [container.get('temperature', 25) for container in container_data]
            avg_temperature = sum(temperatures) / len(temperatures) if temperatures else 25
            ...
            recommendations = []
            if pressure_risk:
                recommendations.append(f"압력 한계 초과 ({pressure_per_m2:.2f} t/m²) - 재배치 필요")
            if temperature_risk:
                recommendations.append(f"온도 한계 초과 ({avg_temperature:.1f}°C) - 냉각 필요")
    ...
    async def execute_real_warehouse_capacity_check(self, warehouse_data: Dict[str, Any]) -> RealMachoGPTResult:
        ...
        try:
            ...
            recommendations = []
            if utilization_risk:
                recommendations.append(f"용량 한계 초과 ({utilization_rate:.1%}) - 공간 확보 필요")
            if temperature_risk:
                recommendations.append(f"온도 한계 초과 ({temperature:.1f}°C) - 냉각 시스템 점검")
            if humidity_risk:
                recommendations.append(f"습도 한계 초과 ({humidity:.1f}%) - 환기 시스템 점검")
...
async def main():
    ...
    try:
        ...
        heat_stow_result = await real_macho.execute_real_heat_stow_analysis(container_data)
        print(f"🔥 Heat-Stow 분석: {heat_stow_result.status} - {heat_stow_result.source}")
        ...
        capacity_result = await real_macho.execute_real_warehouse_capacity_check(warehouse_data)
