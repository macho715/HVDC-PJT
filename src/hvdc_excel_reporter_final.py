...
class PKGValidationEngine:
    ...
    def validate_pkg_accuracy(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            warehouse_columns = ['DHL Warehouse', 'DSV Indoor', 'DSV Al Markaz', 'DSV Outdoor', 'AAA  Storage', 'Hauler Indoor', 'DSV MZP']
            ...
            for _, row in df.iterrows():
                # HVDC CODE 검증
                code1_valid = pd.notna(row['HVDC CODE 1']) and str(row['HVDC CODE 1']).strip() != ''
                code2_valid = pd.notna(row['HVDC CODE 2']) and str(row['HVDC CODE 2']).strip() != ''
                code3_valid = pd.notna(row['HVDC CODE 3']) and str(row['HVDC CODE 3']).strip() != ''
                
                # Category 또는 창고 데이터 검증
                if has_category:
                    category_valid = pd.notna(row['Category']) and str(row['Category']).strip() != ''
                else:
                    # 창고 관련 컬럼 중 하나라도 값이 있으면 유효
                    warehouse_valid = any(
                        pd.notna(row.get(col, '')) and str(row.get(col, '')).strip() != '' 
                        for col in warehouse_columns if col in df.columns
                    )
            ...
            self.logger.info(f"PKG Accuracy: {accuracy:.2%} ({valid_rows}/{total_rows})")
            return result
...
class HVDCExcelReporter:
    ...
    def __init__(self, df: pd.DataFrame, output_file: str = None):
        self.df = df.copy()
        self.output_file = output_file or f"HVDC_Report_{datetime.now().strftime('%Y-%m')}.xlsx"
    ...
    def _load_ontology_rules(self) -> Dict[str, Any]:
        ...
        try:
            ...
            rules = {
                ...
                'required_columns': ['HVDC CODE 1', 'HVDC CODE 2', 'HVDC CODE 3', 'Category', 'Amount', 'Date']
            }
            return rules
        except Exception as e:
    ...
    def normalize_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if 'Category' not in df_clean.columns:
                ...
                def determine_category(row):
                    for col in warehouse_columns:
                        if col in df_clean.columns and pd.notna(row[col]) and str(row[col]).strip() != '':
                            return col
                    return 'Unknown'
            ...
            # 금액 컬럼 처리
            amount_columns = [col for col in df_clean.columns if 'amount' in col.lower() or 'cost' in col.lower() or 'price' in col.lower()]
    ...
    def create_warehouse_performance_sheet(self, writer: pd.ExcelWriter) -> None:
        ...
        try:
            ...
            if 'Category' in df_norm.columns and 'Amount' in df_norm.columns:
                # 창고별 통계
                warehouse_stats = df_norm.groupby('Category').agg({
                    'Amount': ['count', 'sum', 'mean'],
                    'HVDC CODE 3': 'nunique'
                }).round(2)
                
                warehouse_stats.columns = ['Transaction_Count', 'Total_Amount', 'Average_Amount', 'Unique_Items']
                warehouse_stats = warehouse_stats.reset_index()
                
                warehouse_stats.to_excel(writer, sheet_name='Warehouse_Performance', index=False)
                
                self.logger.info("Warehouse performance sheet created")
            else:
                self.logger.warning("Required columns not found for warehouse performance")
    ...
    def create_monthly_trend_sheet(self, writer: pd.ExcelWriter) -> None:
        ...
        try:
            ...
            if date_col and pd.api.types.is_datetime64_any_dtype(df_norm[date_col]):
                ...
                monthly_stats = df_norm.groupby('Month').agg({
                    'Amount': ['count', 'sum'],
                    'Category': 'nunique'
                }).round(2)
                
                monthly_stats.columns = ['Transaction_Count', 'Total_Amount', 'Warehouse_Count']
    ...
    def create_flow_code_analysis_sheet(self, writer: pd.ExcelWriter) -> None:
        ...
        try:
            ...
            if flow_col:
                # Flow Code별 통계
                flow_stats = df_norm.groupby(flow_col).agg({
                    'Amount': ['count', 'sum'],
                    'Category': 'nunique'
                }).round(2)
                
                flow_stats.columns = ['Transaction_Count', 'Total_Amount', 'Warehouse_Count']
                flow_stats = flow_stats.reset_index()
                flow_stats.columns = ['Flow_Code', 'Transaction_Count', 'Total_Amount', 'Warehouse_Count']
                
                flow_stats.to_excel(writer, sheet_name='Flow_Code_Analysis', index=False)
        ...
        try:
            ...
            if code_analysis:
                ...
                for code_col, code_df in list(code_analysis.items())[1:]:
                    result_df = pd.merge(result_df, code_df, left_index=True, right_index=True, how='outer')
                
                result_df.to_excel(writer, sheet_name='HVDC_Code_Breakdown', index=False)
    ...
    def generate_excel_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if pkg_result['status'] == 'FAIL':
                self.logger.error(f"PKG Accuracy below threshold: {pkg_result['accuracy']:.2%}")
                return {
                    'status': 'FAIL',
...
def main():
    ...
    if available_file:
        ...
        sample_data = {
            ...
            'Category': ['DSV Outdoor'] * 40 + ['DSV Indoor'] * 35 + ['DSV Al Markaz'] * 15 + ['DSV MZP'] * 10,
