...
def test_task_operations():
    ...
    try:
        ...
        # 작업 업데이트 테스트
        updated_task = task_manager.update_task(test_task['id'], {
            'status': 'completed',
            'confidence': 0.95
        })
...
def test_macho_gpt_integration():
    ...
    try:
        ...
        if result['status'] == 'SUCCESS':
            print(f"✅ 모드 전환 성공: {result['previous_mode']} → {result['current_mode']}")
        else:
        ...
        if kpi_result['status'] == 'SUCCESS':
            ...
            print(f"❌ LogiMaster KPI 생성 실패: {kpi_result.get('error', 'Unknown error')}")
            return False
        ...
        if stow_result['status'] == 'SUCCESS':
            ...
            print(f"❌ ContainerStow 분석 실패: {stow_result.get('error', 'Unknown error')}")
            return False
        ...
        if weather_result['status'] == 'SUCCESS':
            ...
            print(f"❌ WeatherTie 날씨 확인 실패: {weather_result.get('error', 'Unknown error')}")
            return False
...
def test_task_manager_integration():
    ...
    try:
        ...
        # MACHO-GPT 통합 실행
        integration_result = task_manager.integrate_with_macho_gpt(integration_task['id'])
        ...
        if 'macho_integration' in integration_result:
            ...
            print(f"❌ MACHO-GPT 통합 실패: {integration_result.get('error', 'Unknown error')}")
            return False
