...
class ImprovedWarehouseInboundCalculator:
    ...
    def calculate_monthly_outbound(self, df: pd.DataFrame) -> Dict:
        ...
        warehouse_outbound = (
            outbound_events.groupby(["Month", "Outbound_Flag"])
            .size()
        ...
        # Period를 문자열로 변환
        monthly_result = {
            str(month): count for month, count in monthly_outbound.items()
        }
        ...
        for warehouse in warehouse_cols:
            ...
            for month in monthly_result.keys():
                if warehouse in warehouse_outbound.columns:
                    ...
                    warehouse_monthly_result[warehouse][month] = (
                        warehouse_outbound.loc[month_period, warehouse]
                        if month_period in warehouse_outbound.index
                        else 0
                    )
                else:
        ...
        print(
            f"   ✅ 월별 출고 계산 완료: {len(monthly_result)}개월, {sum(monthly_result.values())}건"
        )
        ...
        return {
            ...
            "outbound_events": outbound_events[
                ["Item", "Date", "Outbound_Flag", "Location"]
            ].to_dict("records"),
        }
    ...
    def calculate_monthly_site_inbound(self, df: pd.DataFrame) -> Dict:
        ...
        site_monthly_inbound = (
            site_inbound_df.groupby(["Month", "Site"]).size().unstack(fill_value=0)
        )

        # Period를 문자열로 변환
        monthly_result = {
            str(month): count for month, count in monthly_site_inbound.items()
        }
        ...
        for site in site_cols:
            ...
            for month in monthly_result.keys():
                if site in site_monthly_inbound.columns:
                    month_period = pd.Period(month)
                    site_monthly_result[site][month] = (
                        site_monthly_inbound.loc[month_period, site]
                        if month_period in site_monthly_inbound.index
                        else 0
                    )
                else:
        ...
        print(
            f"   ✅ 월별 현장 입고 계산 완료: {len(monthly_result)}개월, {sum(monthly_result.values())}건"
        )
    ...
    def calculate_direct_delivery(self, df: pd.DataFrame) -> Dict:
        ...
        for _, row in df.iterrows():
            ...
            has_warehouse = any(
                pd.notna(row.get(col)) for col in self.warehouse_columns if col in row
            )
            has_site = any(
                pd.notna(row.get(col)) for col in self.site_columns if col in row
            )
        ...
        return {"total_direct": len(direct_items), "direct_items": direct_items}
    ...
    def calculate_warehouse_inbound(self, df: pd.DataFrame) -> Dict:
        ...
        monthly_warehouse_inbound = (
            warehouse_inbound_df.groupby("Month").size().to_dict()
        )
        warehouse_monthly_inbound = (
            warehouse_inbound_df.groupby(["Month", "Warehouse"])
            .size()
            .unstack(fill_value=0)
        )
        ...
        # Period를 문자열로 변환
        monthly_result = {
            str(month): count for month, count in monthly_warehouse_inbound.items()
        }
        ...
        for warehouse in self.warehouse_columns:
            ...
            for month in monthly_result.keys():
                if warehouse in warehouse_monthly_inbound.columns:
                    month_period = pd.Period(month)
                    warehouse_monthly_result[warehouse][month] = (
                        warehouse_monthly_inbound.loc[month_period, warehouse]
                        if month_period in warehouse_monthly_inbound.index
                        else 0
                    )
                else:
                    warehouse_monthly_result[warehouse][month] = 0
        ...
        print(
            f"   ✅ 창고 입고 계산 완료: {len(monthly_result)}개월, {sum(monthly_result.values())}건"
        )
    ...
    def calculate_monthly_warehouse_transfer(self, df: pd.DataFrame) -> Dict:
        ...
        # Period를 문자열로 변환
        monthly_result = {
            str(month): count for month, count in monthly_transfer.items()
        }

        print(
            f"   ✅ 월별 창고간 이전 계산 완료: {len(monthly_result)}개월, {sum(monthly_result.values())}건"
        )

        return {
            "monthly_total": monthly_result,
            "total_transfer": sum(monthly_result.values()),
            "transfer_events": warehouse_transfer[
                ["Item", "Date", "Prev_Location", "Location"]
            ].to_dict("records"),
        }
    ...
    def calculate_site_inbound(self, df: pd.DataFrame) -> Dict:
        ...
        print(
            f"   ✅ 현장 입고 계산 완료: {total_site_inbound}건 (창고 경유: {len(site_inbound_items)}, 직송: {direct_delivery['total_direct']})"
        )
    ...
    def validate_monthly_balance(self, df: pd.DataFrame) -> Dict:
        ...
        # 전체 Balance 정확도 계산
        total_balance_ok = sum(
            1 for result in balance_results.values() if result["balance_ok"]
        )
        total_months = len(balance_results)
        balance_accuracy = total_balance_ok / total_months if total_months > 0 else 0

        print(
            f"   ✅ 월별 Balance 검증 완료: {total_balance_ok}/{total_months} 개월 정상 (정확도: {balance_accuracy:.3f})"
        )
    ...
    def generate_comprehensive_report(self, df: pd.DataFrame) -> Dict:
        ...
        outbound_inbound_accuracy = (
            min(outbound_total, site_inbound_total)
            / max(outbound_total, site_inbound_total)
            if max(outbound_total, site_inbound_total) > 0
            else 0
        )
        ...
        print(
            f"   📊 KPI: 출고-입고 일치율 {outbound_inbound_accuracy:.3f}, Balance 정확도 {balance_validation['balance_accuracy']:.3f}"
        )
    ...
    def validate_status_current_field(self, df: pd.DataFrame) -> Dict:
        ...
        status_distribution = (
            df["Status_Current"].value_counts()
        ...
        validation_result = {
            ...
            "consistency_rate": (
                1 - (len(status_inconsistency) / len(warehouse_items))
                if warehouse_items
                else 1
            ),
        }
        ...
        print(
            f"   ⚠️ Status 불일치: {validation_result['status_inconsistency_count']}건"
        )
...
def main():
    ...
    try:
        ...
        print(f"\n📊 최종 결과:")
        print(
            f"   출고-입고 일치율: {report['kpi_metrics']['outbound_inbound_accuracy']:.3f}"
        )
        print(
            f"   월별 Balance 정확도: {report['kpi_metrics']['monthly_balance_accuracy']:.3f}"
        )
        print(
            f"   Fail-safe 권장: {report['zero_mode_recommendation']['switch_to_zero']}"
        )
