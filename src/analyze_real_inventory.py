...
class RealInventoryAnalyzer:
    ...
    def create_realistic_test_data(self):
        ...
        for site in site_cols:
            ...
            for i in range(n_samples):
                ...
                has_warehouse = any(
                    warehouse_dates[wh][i] is not None for wh in warehouse_cols
                )
                if has_warehouse and np.random.random() < 0.7:  # 70% 확률로 현장 입고
                    ...
                    if warehouse_date:
                        site_date = warehouse_date + timedelta(
        ...
        # PKG 수량 (실제 수량 분포)
        pkg_quantities = np.random.choice(
            [1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20],
            size=n_samples,
            p=[0.3, 0.25, 0.2, 0.1, 0.05, 0.03, 0.02, 0.02, 0.01, 0.01, 0.01],
        )

        # 데이터프레임 생성
        data = {"Item": hvdc_codes[:n_samples], "Pkg": pkg_quantities}
    ...
    def analyze_inventory_samples(self, df, sample_size=50):
        ...
        for idx, row in final_df.iterrows():
            # 기본 정보 (실제 컬럼명에 맞게 조정)
            item_column = (
                "Item" if "Item" in row.index else df.columns[0]
            )  # 첫 번째 컬럼을 Item으로 사용

            item_info = {
                "Item": (
                    str(row[item_column])
                    if pd.notna(row[item_column])
                    else f"Item_{idx}"
                ),
            ...
            for warehouse in self.calculator.warehouse_columns:
                if warehouse in row.index and pd.notna(row[warehouse]):
                    ...
                    warehouse_info[warehouse] = {
                        ...
                        "month": (
                            safe_date.strftime("%Y-%m")
                            if not pd.isna(safe_date)
                            else None
                        ),
                    }
            ...
            # 재고 상태 판정
            inventory_status = self.determine_inventory_status(
                row, warehouse_info, site_info
            )
    ...
    def determine_inventory_status(self, row, warehouse_info, site_info):
        ...
        if status == "warehouse":
            ...
            days = (
                (datetime.now() - warehouse_date).days
                if warehouse_date is not None and not pd.isna(warehouse_date)
                else None
            )
            return {
            ...
            days = (
                (datetime.now() - site_date).days
                if site_date is not None and not pd.isna(site_date)
                else None
            )
            return {
                "type": "site_inventory",
    ...
    def calculate_inventory_statistics(self, sample_analysis):
        ...
        for location in warehouse_stats:
            days_list = [
                inv["days"]
                for inv in warehouse_inventory
                if inv["location"] == location and inv["days"] is not None
            ]
            if days_list:
    ...
    def print_inventory_report(self, sample_analysis, statistics):
        ...
        for location, stats in statistics["warehouse_stats"].items():
            ...
            print(
                f"     - 재고 항목: {', '.join(stats['items'][:5])}{'...' if len(stats['items']) > 5 else ''}"
            )
        ...
        print("-" * 100)
        print(
            f"{'Item':<15} {'Pkg':<5} {'Status':<12} {'Location':<15} {'Type':<15} {'Days':<8} {'Description'}"
        )
        print("-" * 100)
        ...
        for i, sample in enumerate(sample_analysis[:10]):
            ...
            days = (
                inventory_status.get("days_in_warehouse")
                or inventory_status.get("days_at_site")
                or "N/A"
            )
            ...
            print(
                f"{item_info['Item']:<15} {item_info['Pkg']:<5} {item_info['Status_Current']:<12} "
        ...
        print(
            f"   창고 재고율: {warehouse_rate:.1f}% ({warehouse_count}/{statistics['total_samples']})"
        )
        ...
        print(
            f"   현장 재고율: {site_rate:.1f}% ({site_count}/{statistics['total_samples']})"
        )
        ...
        print(
            f"   Pre Arrival 비율: {pre_arrival_rate:.1f}% ({pre_arrival_count}/{statistics['total_samples']})"
        )
    ...
    def export_samples_to_excel(
        self, sample_analysis, filename="real_inventory_sample_50.xlsx"
    ):
        """샘플 분석 결과를 엑셀로 저장"""
        rows = []
        for sample in sample_analysis:
            ...
            rows.append(
                {
                    ...
                    "Location": item_info["Status_Location"],
                    "Type": inv["type"],
                    "Days": inv.get("days_in_warehouse")
                    or inv.get("days_at_site")
                    or "N/A",
    ...
    def export_complete_original_data(
        self, df, filename="HVDC_complete_data_original.xlsx"
    ):
        """원본 데이터 전체를 엑셀로 저장"""
