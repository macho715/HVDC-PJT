...
class HVDCComprehensiveReportGenerator:
    ...
    def create_warehouse_io_calculator(self, hitachi_df: pd.DataFrame, siemens_df: pd.DataFrame):
        """입고/출고/재고 계산기 생성"""
        ...
        return pd.DataFrame(warehouse_stats).T.reset_index().rename(columns={'index': 'Warehouse'})
            ...
            for month in range(1, 13):
                ...
                if len(month_data) > 0:
                    monthly_stats.append({
                        ...
                        'Unique_Warehouses': month_data['Status_Location'].nunique(),
    ...
    def create_flow_code_analysis(self, hitachi_df: pd.DataFrame, siemens_df: pd.DataFrame):
        """Flow Code 분석"""
        ...
        flow_stats['Percentage'] = (flow_stats['Count'] / flow_stats['Count'].sum() * 100).round(2)
        
        return flow_stats
    
    def create_warehouse_performance_analysis(self, hitachi_df: pd.DataFrame, siemens_df: pd.DataFrame):
        """창고별 성능 분석"""
        print("📊 창고별 성능 분석 중...")
        ...
        for warehouse in combined_df['Status_Location'].unique():
            ...
            performance = {
                ...
                'Stackable_Items': len(warehouse_data[warehouse_data['Stack'] == 'Stackable']),
                'Non_Stackable_Items': len(warehouse_data[warehouse_data['Stack'] == 'Non Stackable']),
                'Indoor_Items': len(warehouse_data[warehouse_data['Storage'] == 'Indoor']),
                'Outdoor_Items': len(warehouse_data[warehouse_data['Storage'] == 'Outdoor'])
            }
    ...
    def create_excel_report(self, hitachi_df: pd.DataFrame, siemens_df: pd.DataFrame):
        ...
        with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
            ...
            performance_analysis = self.create_warehouse_performance_analysis(hitachi_df, siemens_df)
            performance_analysis.to_excel(writer, sheet_name='07_창고별_성능분석', index=False)
            ...
            quality_report.to_excel(writer, sheet_name='09_데이터_품질리포트', index=False)
        
        print(f"✅ Excel 리포트 생성 완료: {filepath}")
        return filepath
    
    def create_summary_statistics(self, hitachi_df: pd.DataFrame, siemens_df: pd.DataFrame):
        """요약 통계 생성"""
        print("📊 요약 통계 생성 중...")
    ...
    def create_data_quality_report(self, hitachi_df: pd.DataFrame, siemens_df: pd.DataFrame):
        """데이터 품질 리포트 생성"""
        ...
        hitachi_quality = {
            ...
            'Unique_Warehouses': hitachi_df['Status_Location'].nunique(),
            'Date_Range_Start': str(hitachi_df['Status_Location_Date'].min()) if pd.notna(hitachi_df['Status_Location_Date'].min()) else 'N/A',
            'Date_Range_End': str(hitachi_df['Status_Location_Date'].max()) if pd.notna(hitachi_df['Status_Location_Date'].max()) else 'N/A',
        ...
        siemens_quality = {
            ...
            'Unique_Warehouses': siemens_df['Status_Location'].nunique(),
            'Date_Range_Start': str(siemens_df['Status_Location_Date'].min()) if pd.notna(siemens_df['Status_Location_Date'].min()) else 'N/A',
            'Date_Range_End': str(siemens_df['Status_Location_Date'].max()) if pd.notna(siemens_df['Status_Location_Date'].max()) else 'N/A',
