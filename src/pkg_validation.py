...
class PKGValidationSystem:
    ...
    def __init__(self, confidence_threshold: float = 0.95, critical_threshold: int = 50):
        """
        PKG 검증 시스템 초기화
    ...
    def validate_pkg_count_match(self, data1: pd.DataFrame, data2: pd.DataFrame, 
                                data1_name: str = "invoice", data2_name: str = "warehouse") -> Dict[str, Any]:
        """
        PKG 수량 일치 검증
        ...
        try:
            ...
            # 5. 신뢰도 계산
            result['confidence'] = self._calculate_confidence(total1, total2, difference)
            
            # 6. 알림 생성
            self._generate_alerts(result, difference)
            
            # 7. 리포트 생성
            self._generate_report(result, data1_name, data2_name, total1, total2, difference)
            
            self.logger.info(f"PKG 검증 완료: {data1_name} vs {data2_name}, 일치: {result['is_match']}, 차이: {difference}")
    ...
    def _validate_data_integrity(self, data1: pd.DataFrame, data2: pd.DataFrame) -> Dict[str, Any]:
        """데이터 무결성 검사"""
    ...
    def _calculate_confidence(self, total1: float, total2: float, difference: float) -> float:
        """신뢰도 점수 계산"""
        if difference == 0:
    ...
    def _generate_report(self, result: Dict[str, Any], data1_name: str, data2_name: str, 
                        total1: float, total2: float, difference: float):
        """상세 리포트 생성"""
        ...
        result['report']['details'] = {
            ...
            'difference_percentage': round((difference / max(total1, total2)) * 100, 2) if max(total1, total2) > 0 else 0
        }
    ...
    def validate_multiple_sources(self, data_sources: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        """
