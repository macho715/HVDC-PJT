...
def generate_enhanced_excel_report():
    ...
    try:
        ...
        # 데이터 표준화
        hitachi_df["Item"] = hitachi_df.get(
            "HVDC CODE", hitachi_df.get("no.", hitachi_df.index)
        )
        simense_df["Item"] = simense_df.get(
            "HVDC CODE", simense_df.get("No.", simense_df.index)
        )
        ...
        with pd.ExcelWriter(filename, engine="openpyxl") as writer:
            ...
            if "outbound_events" in report["monthly_outbound"]:
                ...
                if not outbound_df.empty:
                    outbound_df.to_excel(
                        writer, sheet_name="Monthly_Outbound", index=False
                    )

            # 3. 월별 현장 입고 시트
            if "site_inbound_items" in report["monthly_site_inbound"]:
                site_inbound_items = report["monthly_site_inbound"][
                    "site_inbound_items"
                ]
                site_inbound_df = pd.DataFrame(site_inbound_items)
                if not site_inbound_df.empty:
                    site_inbound_df.to_excel(
                        writer, sheet_name="Monthly_Site_Inbound", index=False
                    )

            # 4. 직송 상세 시트
            if (
                "direct_delivery" in report["monthly_site_inbound"]
                and "direct_items" in report["monthly_site_inbound"]["direct_delivery"]
            ):
                direct_items = report["monthly_site_inbound"]["direct_delivery"][
                    "direct_items"
                ]
                if direct_items:
                    direct_df = pd.DataFrame(direct_items)
                    direct_df.to_excel(
                        writer, sheet_name="Direct_Delivery", index=False
                    )

            # 5. 창고 재고 시트
            if "inventory_by_location" in report["warehouse_inventory"]:
                inventory_data = []
                for location, items in report["warehouse_inventory"][
                ...
                if inventory_data:
                    ...
                    inventory_df.to_excel(
                        writer, sheet_name="Warehouse_Inventory", index=False
                    )

            # 6. 월별 Balance 검증 시트
            if "monthly_results" in report["balance_validation"]:
                balance_data = []
                for month, result in report["balance_validation"][
                    "monthly_results"
                ].items():
                    balance_data.append(
                        {
                ...
                if balance_data:
                    ...
                    balance_df.to_excel(
                        writer, sheet_name="Monthly_Balance", index=False
                    )
            ...
            if "monthly_total" in report["monthly_outbound"]:
                ...
                for month, count in report["monthly_outbound"]["monthly_total"].items():
                    outbound_monthly_data.append(
                        {"Month": month, "Outbound_Count": count}
                    )

                if outbound_monthly_data:
                    outbound_monthly_df = pd.DataFrame(outbound_monthly_data)
                    outbound_monthly_df.to_excel(
                        writer, sheet_name="Monthly_Outbound_Summary", index=False
                    )

            # 8. 월별 현장 입고 요약 시트
            if "monthly_total" in report["monthly_site_inbound"]:
                site_monthly_data = []
                for month, count in report["monthly_site_inbound"][
                    "monthly_total"
                ].items():
                    site_monthly_data.append(
                        {"Month": month, "Site_Inbound_Count": count}
                    )

                if site_monthly_data:
                    site_monthly_df = pd.DataFrame(site_monthly_data)
                    site_monthly_df.to_excel(
                        writer, sheet_name="Monthly_Site_Inbound_Summary", index=False
                    )
