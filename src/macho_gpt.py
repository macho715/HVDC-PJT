...
class ContainerStow:
    ...
    def heat_stow_analysis(self, container_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Heat-based stowage pressure analysis"""
        try:
            # 시뮬레이션된 Heat-Stow 분석
            total_pressure = sum(container.get('weight', 0) for container in container_data)
            ...
            result = {
                ...
                'confidence': 0.96,
                'mode': 'LATTICE',
                'triggers': ['/pressure_check' if pressure_per_m2 > self.pressure_limit else None],
            ...
            self.logger.info(f"Heat-Stow analysis completed: {pressure_per_m2:.2f} t/m²")
            return result
        except Exception as e:
    ...
    def whf_capacity_check(self, warehouse_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if utilization_rate > 0.85:
                result['data']['recommendations'].append('창고 용량 85% 초과 - 공간 확보 필요')
...
class WeatherTie:
    ...
    def check_weather_conditions(self, port_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if impact_score > 0.3:
                result['data']['recommendations'].append('날씨 영향 감지 - ETA 조정 권장')
                
            self.logger.info(f"Weather check completed for {port_code}: {impact_score:.2f}")
            return result
        except Exception as e:
