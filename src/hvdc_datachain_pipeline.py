...
class HVDCDataChainPipeline:
    ...
    def _build_pipeline(self) -> object:
        ...
        try:
            ...
            preprocess_chain = (
                excel_source
                .map(self._validate_excel_structure, output=["valid", "data", "errors"])
                .filter(lambda x: x[0])  # 유효한 데이터만 필터링
                .map(self._extract_hvdc_data, output=["warehouse_data", "invoice_data", "status_data"])
                .map(self._clean_and_validate_data, output=["cleaned_data", "validation_report"])
            )
            
            # 4. KPI 계산 체인
            kpi_chain = (
                preprocess_chain
                .map(self._calculate_kpis, output=["kpi_results", "confidence_score"])
                .filter(lambda x: x[1] >= self.confidence_threshold)  # 신뢰도 임계값 필터링
            ...
            logistics_chain = (
                kpi_chain
                .map(self._analyze_logistics_metrics, output=["logistics_analysis", "optimization_suggestions"])
                .map(self._apply_fanr_compliance, output=["compliance_status", "compliance_report"])
                .map(self._generate_final_report, output=["final_report", "next_actions"])
            )
    ...
    def _validate_excel_structure(self, file_path: str) -> Tuple[bool, Dict, List[str]]:
        ...
        try:
            ...
            if 'WAREHOUSE' in file_path.upper():
                missing_cols = [col for col in required_columns['warehouse'] 
                              if col not in df.columns]
                if missing_cols:
                    errors.append(f"Missing warehouse columns: {missing_cols}")
            
            elif 'INVOICE' in file_path.upper():
                missing_cols = [col for col in required_columns['invoice'] 
                              if col not in df.columns]
                if missing_cols:
                    errors.append(f"Missing invoice columns: {missing_cols}")
            
            elif 'STATUS' in file_path.upper():
                missing_cols = [col for col in required_columns['status'] 
                              if col not in df.columns]
                if missing_cols:
                    errors.append(f"Missing status columns: {missing_cols}")
            
            valid = len(errors) == 0
            logger.info(f"Excel validation for {file_path}: {'PASS' if valid else 'FAIL'}")
    ...
    def _calculate_kpis(self, data_tuple: Tuple) -> Tuple[Dict, float]:
        ...
        try:
            ...
            if 'warehouse' in cleaned_data:
                ...
                # 총 PKG 수
                total_pkg = warehouse_df['PKG'].sum() if 'PKG' in warehouse_df.columns else 0
                
                # 총 QTY
                total_qty = warehouse_df['QTY'].sum() if 'QTY' in warehouse_df.columns else 0
                
                # 위치별 분포
                location_distribution = warehouse_df['LOCATION'].value_counts().to_dict() if 'LOCATION' in warehouse_df.columns else {}
                
                # 상태별 분포
                status_distribution = warehouse_df['STATUS'].value_counts().to_dict() if 'STATUS' in warehouse_df.columns else {}
            ...
            if 'invoice' in cleaned_data:
                ...
                # 총 금액
                total_amount = invoice_df['AMOUNT'].sum() if 'AMOUNT' in invoice_df.columns else 0
                
                # HS 코드별 분포
                hs_distribution = invoice_df['HS_CODE'].value_counts().to_dict() if 'HS_CODE' in invoice_df.columns else {}
            ...
            if 'status' in cleaned_data:
                ...
                # 최신 상태
                latest_status = status_df.iloc[-1].to_dict() if len(status_df) > 0 else {}
            ...
            logger.info(f"KPI calculation completed with confidence: {final_confidence:.2f}")
    ...
    def _generate_kpi_report(self, data_tuple: Tuple) -> Tuple[Dict, List[str]]:
        ...
        try:
            ...
            if confidence_score < self.confidence_threshold:
                alerts.append(f"LOW_CONFIDENCE: 신뢰도 점수가 {self.confidence_threshold} 미만입니다.")
            
            # 권장사항 생성
            if alerts:
                kpi_report['recommendations'].append("즉시 조치가 필요한 알림이 있습니다.")
            
            if confidence_score >= 0.95:
                kpi_report['recommendations'].append("데이터 품질이 우수합니다. 자동화된 처리를 진행할 수 있습니다.")
    ...
    def _analyze_logistics_metrics(self, data_tuple: Tuple) -> Tuple[Dict, List[str]]:
        ...
        try:
            ...
            if 'warehouse' in metrics:
                ...
                if utilization > 90:
                    optimization_suggestions.append("창고 용량 확장 또는 추가 창고 검토가 필요합니다.")
                elif utilization < 50:
                    optimization_suggestions.append("창고 공간 최적화를 위한 재배치를 고려하세요.")
            ...
            logger.info(f"Logistics analysis completed: {logistics_analysis['analysis_id']}")
    ...
    def _apply_fanr_compliance(self, data_tuple: Tuple) -> Tuple[Dict, Dict]:
        ...
        try:
            ...
            if warehouse_analysis.get('utilization_rate', 0) > 95:
                ...
                compliance_report['violations'].append("창고 활용률이 안전 한계를 초과했습니다.")
            else:
            ...
            logger.info(f"FANR compliance check completed: {compliance_status['compliance_id']}")
    ...
    def _generate_final_report(self, data_tuple: Tuple) -> Tuple[Dict, List[str]]:
        ...
        try:
            ...
            final_report = {
                ...
                'mode': self.mode,
                'executive_summary': {
                    'overall_status': 'SUCCESS' if compliance_status.get('overall_compliance', False) else 'ATTENTION_REQUIRED',
                    'compliance_score': self._calculate_compliance_score(compliance_status),
                    'data_quality_score': compliance_status.get('overall_compliance', False) * 100
                },
                'detailed_analysis': {
            ...
            if not compliance_status.get('overall_compliance', True):
                final_report['system_recommendations'].append("즉시 조치가 필요한 규정 위반이 발견되었습니다.")
                next_actions.append("IMMEDIATE_ACTION: 규정 위반 사항 즉시 조치")
            
            if compliance_status.get('overall_compliance', False):
                final_report['system_recommendations'].append("모든 규정을 준수하고 있습니다. 정기 모니터링을 계속하세요.")
    ...
    def run_pipeline(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'timestamp': datetime.now().isoformat(),
                'confidence': final_report.get('executive_summary', {}).get('data_quality_score', 0)
            }
