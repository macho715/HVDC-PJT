...
def check_enhanced_excel_report():
    ...
    excel_files = [
        ...
        if f.startswith("HVDC_Enhanced_Report_") and f.endswith(".xlsx")
    ]
    ...
    # 가장 최근 파일 선택
    latest_file = max(
        excel_files, key=lambda x: os.path.getmtime(os.path.join(output_dir, x))
    )
    ...
    try:
        ...
        if "창고_월별_입출고" in excel_data:
            ...
            print(
                f"✅ 창고_월별_입출고: {len(warehouse_data)}행 × {len(warehouse_data.columns)}열"
            )
        ...
        if "현장_월별_입고재고" in excel_data:
            ...
            print(
                f"✅ 현장_월별_입고재고: {len(site_data)}행 × {len(site_data.columns)}열"
            )
