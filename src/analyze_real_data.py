...
def check_compatibility_with_calculator(df):
    ...
    if missing_cols:
        ...
        # 대체 컬럼 찾기
        potential_item_cols = [
            col for col in df.columns if "item" in col.lower() or "id" in col.lower()
        ]
        potential_status_cols = [
            col
            for col in df.columns
            if "status" in col.lower() or "current" in col.lower()
        ]
...
def main():
    ...
    if hitachi_df is not None:
        warehouse_cols, site_cols, date_cols = identify_warehouse_and_site_columns(
            hitachi_df
        )
