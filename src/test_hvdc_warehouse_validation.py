...
# 테스트 모듈 임포트
from test_warehouse_io_validation import WarehouseIOCalculator, DataQualityValidator, ComprehensiveWarehouseValidator
...
def test_hvdc_warehouse_validation():
    ...
    print(f"📥 창고별 입고: {dict(list(inbound_result['by_warehouse'].items())[:3])}...")
    ...
    print(f"🔢 WH_HANDLING 검증: {'통과' if wh_validation['validation_passed'] else '실패'}")
    print(f"🔢 허용 오차 초과: {'예' if wh_validation['tolerance_exceeded'] else '아니오'}")
    ...
    comprehensive_result = comprehensive_validator.validate_comprehensive_warehouse_data(result_df)
        ...
        if isinstance(error_details, dict):
            for level, details in error_details.items():
                if isinstance(details, dict):
                    print(f"   Level {level}: 우리={details['our_count']}, Excel={details['excel_count']}, 차이={details['difference']}")
...
def generate_validation_report(results):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 30 + "\n")
        f.write(f"Excel 대조 검증: {results['excel_validation']['validation_status']}\n")
        f.write(f"정확도 점수: {results['excel_validation']['accuracy_score']:.2%}\n")
        f.write(f"WH_HANDLING 검증: {'통과' if results['wh_validation']['validation_passed'] else '실패'}\n\n")
        ...
        f.write("컴포넌트 점수:\n")
        for component, score in results['comprehensive_result']['component_scores'].items():
