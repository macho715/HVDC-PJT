...
class EnhancedHVDCContextScoring:
    ...
    def calculate_context_precision(self, context: EnhancedHVDCContextWindow) -> float:
        ...
        relevant_keywords = ["HVDC", "HITACHI", "SIEMENS", "warehouse", "logistics", "FANR", "MOIAT"]
        relevant_count = 0
    ...
    def calculate_context_recall(self, context: EnhancedHVDCContextWindow) -> float:
        ...
        relevant_retrieved = len([ex for ex in context.examples if self._is_relevant_example(ex)])
    ...
    def calculate_memory_quality(self, context: EnhancedHVDCContextWindow) -> Dict[str, float]:
        ...
        freshness = recent_memory_count / total_memory_count if total_memory_count > 0 else 0.0
        ...
        relevance = relevant_memory_count / total_memory_count if total_memory_count > 0 else 0.0
    ...
    def score_context_quality_enhanced(self, context: EnhancedHVDCContextWindow) -> float:
        ...
        memory_score = (memory_quality["freshness"] + 
                       memory_quality["relevance"] + 
                       memory_quality["coherence"]) / 3
    ...
    def update_dynamic_thresholds(self, recent_scores: List[float]):
        ...
        # 새로운 임계값 설정
        self.dynamic_thresholds["confidence"] = max(0.7, min(0.95, mean_score + std_score))
        
        self.logger.info(f"동적 임계값 업데이트: confidence = {self.dynamic_thresholds['confidence']:.3f}")
    ...
    def _is_relevant_memory(self, key: str, value: Any) -> bool:
        ...
        return (any(keyword.lower() in key_lower for keyword in relevant_keywords) or
                any(keyword.lower() in value_text for keyword in relevant_keywords))
    ...
    def _calculate_domain_grounding(self, context: EnhancedHVDCContextWindow) -> float:
        """도메인 그라운딩 계산"""
        domain_keywords = ["HVDC", "HITACHI", "SIEMENS", "UAE", "logistics", "warehouse"]
        context_text = f"{context.prompt} {str(context.logistics_context)} {str(context.fanr_compliance)}"
        context_lower = context_text.lower()
        
        found_keywords = sum(1 for keyword in domain_keywords if keyword.lower() in context_lower)
    ...
    def _calculate_response_completeness(self, response: Dict[str, Any]) -> float:
        ...
        required_score = sum(1 for field in required_fields if field in response) / len(required_fields)
        optional_score = sum(1 for field in optional_fields if field in response) / len(optional_fields)
...
class EnhancedHVDCContextProtocol:
    ...
    async def create_context_for_command(self, command: str, parameters: Dict[str, Any]) -> EnhancedHVDCContextWindow:
        ...
        if command == "enhance_dashboard":
            ...
            context.examples = [
                {"dashboard_id": "main", "enhancement_type": "weather_integration", "result": "success", "metrics": {"performance": 0.95}},
                {"dashboard_id": "kpi", "enhancement_type": "kpi_monitoring", "result": "success", "metrics": {"efficiency": 0.92}}
            ]
            context.tools = ["weather_api", "kpi_database", "html_generator", "performance_monitor"]
            
        elif command == "excel_query":
            context.prompt = f"Excel 자연어 쿼리: {parameters.get('query')}"
            context.examples = [
                {"query": "Show me all Hitachi equipment", "result": "filtered_data", "precision": 0.95, "recall": 0.88},
                {"query": "Calculate total weight by vendor", "result": "aggregated_data", "precision": 0.92, "recall": 0.85}
            ]
            context.tools = ["pandas", "excel_parser", "natural_language_processor", "data_validator"]
            
        elif command == "weather_tie":
            context.prompt = f"기상 연동 분석: {parameters.get('weather_data')}"
            context.examples = [
                {"weather": "storm", "eta_delay": "24h", "action": "update_eta", "confidence": 0.95},
                {"weather": "normal", "eta_delay": "0h", "action": "no_change", "confidence": 0.98}
            ]
            context.tools = ["weather_api", "eta_calculator", "notification_system", "risk_assessor"]
            
        elif command == "get_kpi":
            context.prompt = f"KPI 조회: {parameters.get('metric')}"
            context.examples = [
                {"metric": "efficiency", "value": 0.92, "trend": "increasing", "threshold": 0.85},
                {"metric": "performance", "value": 0.88, "trend": "stable", "threshold": 0.80}
            ]
            ...
            context.examples = [
                {"mode": "LATTICE", "reason": "OCR_processing", "success": True, "latency": 0.5},
                {"mode": "PRIME", "reason": "normal_operation", "success": True, "latency": 0.2}
            ]
    ...
    async def update_context_with_response(self, context: EnhancedHVDCContextWindow, response: Dict[str, Any]):
        ...
        # 동적 임계값 업데이트
        self.scoring.update_dynamic_thresholds(self.performance_metrics["context_scores"])
    ...
    def _update_performance_metrics(self, context: EnhancedHVDCContextWindow, response: Dict[str, Any]):
        """성능 메트릭 업데이트"""
        ...
        self.performance_metrics["precision_scores"].append(getattr(context, 'context_precision', 0.0))
        self.performance_metrics["recall_scores"].append(getattr(context, 'context_recall', 0.0))
        self.performance_metrics["groundedness_scores"].append(getattr(context, 'groundedness', 0.0))
        
        # 메트릭 크기 제한
        max_metrics_size = 100
        for key in self.performance_metrics:
            if len(self.performance_metrics[key]) > max_metrics_size:
                self.performance_metrics[key] = self.performance_metrics[key][-max_metrics_size:]
...
class EnhancedHVDCContextEngineeringIntegration:
    ...
    async def execute_command_with_context(self, command: str, parameters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            # 1. Context Window 생성
            context = await self.protocol.create_context_for_command(command, parameters or {})
            
            # 2. 업계 표준 Context 품질 평가
            context_score = self.protocol.scoring.score_context_quality_enhanced(context)
            ...
            # 5. 업계 표준 응답 품질 평가
            response_score = self.protocol.scoring.score_response_quality_enhanced(result)
    ...
    async def get_enhanced_context_analytics(self) -> Dict[str, Any]:
        ...
        # 업계 표준 지표
        precision_scores = [getattr(ctx, 'context_precision', 0.0) for ctx in recent_contexts]
        recall_scores = [getattr(ctx, 'context_recall', 0.0) for ctx in recent_contexts]
        groundedness_scores = [getattr(ctx, 'groundedness', 0.0) for ctx in recent_contexts]
        
        # 메모리 품질
        memory_freshness = [getattr(ctx, 'memory_freshness', 0.0) for ctx in recent_contexts]
        memory_relevance = [getattr(ctx, 'memory_relevance', 0.0) for ctx in recent_contexts]
        memory_coherence = [getattr(ctx, 'memory_coherence', 0.0) for ctx in recent_contexts]
        
        return {
            "total_contexts": len(self.protocol.context_history),
            "average_context_score": np.mean(context_scores) if context_scores else 0,
            "average_response_score": np.mean(response_scores) if response_scores else 0,
            ...
            "average_groundedness": np.mean(groundedness_scores) if groundedness_scores else 0,
            "field_resonance_trend": [ctx.field_resonance for ctx in recent_contexts],
            "attractor_strength_trend": [ctx.attractor_strength for ctx in recent_contexts],
            "memory_quality": {
                "average_freshness": np.mean(memory_freshness) if memory_freshness else 0,
                "average_relevance": np.mean(memory_relevance) if memory_relevance else 0,
                "average_coherence": np.mean(memory_coherence) if memory_coherence else 0
            },
