...
class TestStatusCalculatorIntegration:
    ...
    def test_load_hitachi_data_and_calculate_status(self):
        ...
        assert all(status in valid_statuses for status in result_df['Status_Current'].unique())
    ...
    def test_combined_data_status_calculation(self):
        ...
        assert expected_range[0] <= len(result_df) <= expected_range[1], \
    ...
    def test_warehouse_inbound_pattern_analysis(self):
        ...
        print(f"\n🏢 창고별 입고 패턴:")
        for warehouse, count in sorted(warehouse_inbound_summary.items(), 
        ...
        for warehouse in major_warehouses:
            if warehouse in warehouse_inbound_summary:
                assert warehouse_inbound_summary[warehouse] > 0, \
    ...
    def test_pre_arrival_location_priority(self):
        ...
        test_data = pd.DataFrame({
            ...
            'DSV Al Markaz': [datetime(2024, 3, 15), datetime(2024, 5, 20), datetime(2024, 4, 10)],
            'MIR': [None, None, None],
            'SHU': [None, None, None],
            'DAS': [None, None, None], 
            'AGI': [None, None, None]
        ...
        assert result_df.iloc[0]['Status_Location'] == 'DSV Al Markaz'  # 동일값→Al Markaz 우선
        
        assert result_df.iloc[1]['Status_Current'] == 'Pre Arrival'
        assert result_df.iloc[1]['Status_Location'] == 'DSV Al Markaz'  # 최대값→Al Markaz
        
        assert result_df.iloc[2]['Status_Current'] == 'Pre Arrival'
        assert result_df.iloc[2]['Status_Location'] == 'DSV Al Markaz'  # 하나만 존재→Al Markaz
