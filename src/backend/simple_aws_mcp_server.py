    ...
    try:
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps(
                    {"status": "success", "buckets": buckets, "count": len(buckets)},
                    indent=2,
                    ensure_ascii=False,
                ),
            )
        ...
        return [TextContent(type="text", text=f"S3 버킷 조회 오류: {str(e)}")]
    ...
    try:
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps(
                    {"status": "success", "account_info": response},
                    indent=2,
                    ensure_ascii=False,
                ),
            )
        ...
        return [TextContent(type="text", text=f"계정 정보 조회 오류: {str(e)}")]
...
async def list_regions() -> List[TextContent]:
    ...
    try:
        ...
        for region in response["Regions"]:
            regions.append(
                {"RegionName": region["RegionName"], "Endpoint": region["Endpoint"]}
            )
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps(
                    {"status": "success", "regions": regions, "count": len(regions)},
                    indent=2,
                    ensure_ascii=False,
                ),
            )
        ...
        return [TextContent(type="text", text=f"리전 조회 오류: {str(e)}")]
...
async def main():
    ...
    logger.info(
        f"AWS Access Key: {aws_access_key[:8] if aws_access_key else 'Not set'}..."
    )
