...
class ValidationReportGenerator:
    ...
    def generate_validation_report(self, invoice_file: str, warehouse_file: str) -> Dict[str, Any]:
        """
        종합 검증 리포트 생성
        ...
        try:
            ...
            # 3. 교차 검증 수행
            cross_validation = self._perform_cross_validation(invoice_data, warehouse_data)
            
            # 4. 규정 준수 검증
            compliance_validation = self._validate_compliance(invoice_data, warehouse_data)
            
            # 5. 전체 검증 결과 통합
            overall_validation = self._calculate_overall_validation(
                invoice_validation, warehouse_validation, cross_validation, compliance_validation
            )
            
            # 6. 리포트 파일 생성
            generated_files = self._generate_report_files(
                timestamp, invoice_validation, warehouse_validation, 
                cross_validation, compliance_validation, overall_validation
            )
            ...
            logger.info(f"✅ 검증 리포트 생성 완료 - 전체 점수: {overall_validation['total_score']:.3f}")
            return result
    ...
    def _load_invoice_data(self, invoice_file: str) -> pd.DataFrame:
        ...
        try:
            ...
            # 기본 데이터 검증
            required_columns = ['Item', 'Description', 'Quantity', 'Unit_Price', 'Total_Amount']
    ...
    def _clean_invoice_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 매핑 적용 (우선순위 고려)
        for mapping_dict in [item_mappings, desc_mappings, qty_mappings, price_mappings, total_mappings]:
            for old_col, new_col in mapping_dict.items():
                if old_col in df_clean.columns and new_col not in column_mapping.values():
        ...
        if 'Amount' in df_clean.columns and 'Amount' not in column_mapping:
            ...
            if 'Quantity' not in df_clean.columns and 'Total_Amount' not in df_clean.columns:
        ...
        if 'Total_Amount' in df_clean.columns:
            ...
            total_amount_cols = [col for col in df_clean.columns if col == 'Total_Amount']
        ...
        if 'Unit_Price' not in df_clean.columns:
            ...
            logger.info(f"🔧 Unit_Price 컬럼이 이미 존재함: {df_clean['Unit_Price'].notna().sum()}개 유효값")
        ...
        for col in numeric_columns:
            if col in df_clean.columns:
                ...
                logger.info(f"📊 {col} 컬럼 정제 완료: {df_clean[col].notna().sum()}개 유효값")
    ...
    def _clean_warehouse_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 창고 컬럼 식별 및 출력
        warehouse_columns = [col for col in df_clean.columns if any(warehouse in col for warehouse in 
                           ['DSV', 'DHL', 'AAA', 'Hauler', 'MOSB', 'MIR', 'SHU', 'DAS', 'AGI'])]
    ...
    def _debug_dataframe_structure(self, df: pd.DataFrame, data_type: str):
        ...
        for col in df.columns:
            try:
                ...
                logger.info(f"    {col}: {dtype} (유효: {non_null_count}, 빈값: {null_count})")
            except Exception as e:
        ...
        if data_type == "청구서 데이터":
            required_cols = ['Item', 'Description', 'Quantity', 'Unit_Price', 'Total_Amount']
        else:  # 창고 데이터
            required_cols = ['Item']
    ...
    def _validate_invoice_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(df) > 0:
            passed_checks += 1
            validation_result['checks']['data_structure'] = {'status': 'PASS', 'message': '데이터 구조 정상'}
        else:
            validation_result['checks']['data_structure'] = {'status': 'FAIL', 'message': '빈 데이터'}
        ...
        for col in required_columns:
            ...
            if col in df.columns:
                passed_checks += 1
                validation_result['checks'][f'column_{col}'] = {'status': 'PASS', 'message': f'{col} 컬럼 존재'}
            else:
                validation_result['checks'][f'column_{col}'] = {'status': 'FAIL', 'message': f'{col} 컬럼 누락'}
        ...
        if null_count == 0:
            passed_checks += 1
            validation_result['checks']['data_quality'] = {'status': 'PASS', 'message': '데이터 품질 양호'}
        else:
            validation_result['checks']['data_quality'] = {'status': 'WARNING', 'message': f'{null_count}개 빈 값 발견'}
        ...
        if 'Quantity' in df.columns and 'Unit_Price' in df.columns and 'Total_Amount' in df.columns:
            ...
            if accuracy_rate >= 0.95:
                ...
                validation_result['errors'].append(f'금액 계산 오차 {1-accuracy_rate:.1%}')
        
        # 점수 계산
        validation_result['score'] = passed_checks / total_checks if total_checks > 0 else 0
        validation_result['status'] = 'PASS' if validation_result['score'] >= 0.9 else 'FAIL'
    ...
    def _validate_warehouse_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 4. 창고 컬럼 검증
        warehouse_columns = [col for col in df.columns if any(warehouse in col for warehouse in 
                           ['DSV', 'DHL', 'AAA', 'Hauler', 'MOSB', 'MIR', 'SHU', 'DAS', 'AGI'])]
        ...
        logger.info(f"✅ 창고 데이터 검증 완료 - 점수: {validation_result['score']:.3f}")
        return validation_result
    
    def _perform_cross_validation(self, invoice_df: pd.DataFrame, warehouse_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        match_rate = len(matched_items) / len(invoice_items) if len(invoice_items) > 0 else 0
        ...
        if len(matched_items) > 0:
            ...
            matched_invoice = invoice_df[invoice_df['Item'].astype(str).isin(matched_items)]
            matched_warehouse = warehouse_df[warehouse_df['Item'].astype(str).isin(matched_items)]
            
            # HVDC CODE 컬럼 찾기
            invoice_hvdccode_col = 'HVDC CODE' if 'HVDC CODE' in matched_invoice.columns else 'Description'
            warehouse_hvdccode_col = 'HVDC CODE' if 'HVDC CODE' in matched_warehouse.columns else 'Description'
            ...
            for _, invoice_row in matched_invoice.iterrows():
                ...
                warehouse_items_for_id = matched_warehouse[matched_warehouse['Item'].astype(str) == item_id]
                
                if len(warehouse_items_for_id) > 0:
                    warehouse_hvdccode = normalize_hvdccode(warehouse_items_for_id.iloc[0][warehouse_hvdccode_col])
                    ...
                    if invoice_hvdccode == warehouse_hvdccode:
                        ...
                        mismatch_details.append({
                            'item': item_id,
                            'invoice_hvdccode_original': str(invoice_row[invoice_hvdccode_col]),
                            'warehouse_hvdccode_original': str(warehouse_items_for_id.iloc[0][warehouse_hvdccode_col]),
                            'invoice_hvdccode_normalized': invoice_hvdccode,
                            'warehouse_hvdccode_normalized': warehouse_hvdccode
                        })
            
            hvdccode_match_rate = hvdccode_matches / len(matched_invoice) if len(matched_invoice) > 0 else 0
        ...
        if len(matched_items) > 0:
            ...
            # 수량 컬럼 찾기
            invoice_qty_col = 'Quantity' if 'Quantity' in matched_invoice.columns else 'pkg'
            ...
            for _, invoice_row in matched_invoice.iterrows():
                ...
                if invoice_qty <= 5:
                    ...
                    qty_tolerance = max(2, int(invoice_qty * 0.1))  # 6-20개: 10% 또는 2개
                elif invoice_qty <= 100:
                    qty_tolerance = max(3, int(invoice_qty * 0.08))  # 21-100개: 8% 또는 3개
                else:
                    qty_tolerance = max(5, int(invoice_qty * 0.05))  # 100개 초과: 5% 또는 5개
                ...
                if qty_difference <= qty_tolerance:
                    ...
                    qty_mismatch_details.append({
                        ...
                        'tolerance_rate': f"{qty_tolerance/invoice_qty*100:.1f}%" if invoice_qty > 0 else "N/A"
                    })
            
            qty_match_rate = qty_matches / len(matched_invoice) if len(matched_invoice) > 0 else 0
        ...
        # 4. 데이터 완전성 검증
        total_checks += 1
        invoice_completeness = (matched_invoice.notna().sum().sum() / (len(matched_invoice) * len(matched_invoice.columns))) * 100 if len(matched_invoice) > 0 else 0
        warehouse_completeness = (matched_warehouse.notna().sum().sum() / (len(matched_warehouse) * len(matched_warehouse.columns))) * 100 if len(matched_warehouse) > 0 else 0
        ...
        total_checks += 1
        # 날짜 컬럼 찾기
        invoice_date_cols = [col for col in matched_invoice.columns if 'date' in col.lower() or 'month' in col.lower()]
        warehouse_date_cols = [col for col in matched_warehouse.columns if 'date' in col.lower() or 'month' in col.lower()]
        
        if invoice_date_cols and warehouse_date_cols:
            # 청구서 날짜 범위
            invoice_dates = pd.to_datetime(matched_invoice[invoice_date_cols[0]], errors='coerce').dropna()
            invoice_date_range = (invoice_dates.min(), invoice_dates.max()) if len(invoice_dates) > 0 else (None, None)
            
            # 창고 날짜 범위
            warehouse_dates = pd.to_datetime(matched_warehouse[warehouse_date_cols[0]], errors='coerce').dropna()
            warehouse_date_range = (warehouse_dates.min(), warehouse_dates.max()) if len(warehouse_dates) > 0 else (None, None)
            ...
            if invoice_date_range[0] and warehouse_date_range[0]:
                ...
                overlap_range = (overlap_start, overlap_end) if overlap else (None, None)
        ...
        logger.info(f"  📊 매칭된 아이템: {len(matched_items)}/{len(invoice_items)} ({match_rate:.1%})")
        logger.info(f"  📋 청구서 전용: {len(invoice_only)}건, 창고 전용: {len(warehouse_only)}건")
        
        return validation_result
    
    def _validate_compliance(self, invoice_df: pd.DataFrame, warehouse_df: pd.DataFrame) -> Dict[str, Any]:
        """규정 준수 검증 - 비활성화됨"""
        ...
        logger.info(f"✅ 규정 준수 검증 비활성화 - 점수: {validation_result['score']:.3f}")
        return validation_result
    
    def _calculate_overall_validation(self, invoice_validation: Dict, warehouse_validation: Dict, 
                                    cross_validation: Dict, compliance_validation: Dict) -> Dict[str, Any]:
        """전체 검증 결과 계산"""
        ...
        # 요약 통계 계산
        all_validations = [invoice_validation, warehouse_validation, cross_validation, compliance_validation]
        ...
        logger.info(f"✅ 전체 검증 결과 계산 완료 - 점수: {total_score:.3f}, 등급: {grade}")
        return overall_validation
    
    def _generate_report_files(self, timestamp: str, invoice_validation: Dict, warehouse_validation: Dict,
                              cross_validation: Dict, compliance_validation: Dict, overall_validation: Dict) -> Dict[str, str]:
        """리포트 파일 생성"""
        ...
        if self.config.generate_pdf and REPORTLAB_AVAILABLE:
            pdf_file = self._generate_pdf_report(timestamp, invoice_validation, warehouse_validation,
                                               cross_validation, compliance_validation, overall_validation)
            generated_files['pdf_summary'] = str(pdf_file)
        elif self.config.generate_pdf and not REPORTLAB_AVAILABLE:
            logger.warning("PDF 보고서 생성이 비활성화되었습니다 (reportlab 패키지 없음)")
        
        # 2. Excel 상세 결과 생성
        if self.config.generate_excel:
            excel_file = self._generate_excel_report(timestamp, invoice_validation, warehouse_validation,
                                                   cross_validation, compliance_validation, overall_validation)
            generated_files['excel_detailed'] = str(excel_file)
        
        # 3. RDF TTL 파일 생성
        if self.config.generate_rdf and RDF_AVAILABLE:
            rdf_file = self._generate_rdf_report(timestamp, invoice_validation, warehouse_validation,
                                               cross_validation, compliance_validation, overall_validation)
            generated_files['rdf_ontology'] = str(rdf_file)
        elif self.config.generate_rdf and not RDF_AVAILABLE:
            logger.warning("RDF TTL 파일 생성이 비활성화되었습니다 (rdflib 패키지 없음)")
        
        logger.info(f"✅ 리포트 파일 생성 완료: {len(generated_files)}개 파일")
        return generated_files
    
    def _generate_pdf_report(self, timestamp: str, invoice_validation: Dict, warehouse_validation: Dict,
                           cross_validation: Dict, compliance_validation: Dict, overall_validation: Dict) -> Path:
        """PDF 요약 보고서 생성"""
        ...
        summary_data = [
            ['검증 항목', '상태', '점수', '등급'],
            ['전체 검증', overall_validation['status'], f"{overall_validation['total_score']:.3f}", overall_validation['grade']],
            ['청구서 검증', invoice_validation['status'], f"{invoice_validation['score']:.3f}", ''],
            ['창고 검증', warehouse_validation['status'], f"{warehouse_validation['score']:.3f}", ''],
            ['교차 검증', cross_validation['status'], f"{cross_validation['score']:.3f}", ''],
            ['규정 준수', compliance_validation['status'], f"{compliance_validation['score']:.3f}", '']
        ]
        ...
        for validation_name, validation_data in [
            ...
            story.append(Paragraph(f"{validation_name}: {validation_data['status']}", styles['Heading3']))
            
            for check_name, check_result in validation_data['checks'].items():
                status_color = 'green' if check_result['status'] == 'PASS' else 'red' if check_result['status'] == 'FAIL' else 'orange'
                story.append(Paragraph(
                    f"• {check_name}: {check_result['status']} - {check_result['message']}",
                    ParagraphStyle('CheckResult', parent=styles['Normal'], textColor=colors.HexColor(status_color))
                ))
    ...
    def _generate_excel_report(self, timestamp: str, invoice_validation: Dict, warehouse_validation: Dict,
                             cross_validation: Dict, compliance_validation: Dict, overall_validation: Dict) -> Path:
        """Excel 상세 결과 생성"""
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            ...
            pd.DataFrame(invoice_details).to_excel(writer, sheet_name='청구서_검증', index=False)
            ...
            pd.DataFrame(warehouse_details).to_excel(writer, sheet_name='창고_검증', index=False)
            ...
            pd.DataFrame(cross_details).to_excel(writer, sheet_name='교차_검증', index=False)
            ...
            if 'details' in cross_validation:
                ...
                if 'item_matching' in cross_validation['details']:
                    ...
                    cross_stats.extend([
                        {'통계 항목': '아이템 매칭 - 총 청구서 아이템', '값': item_stats['total_invoice_items']},
                        {'통계 항목': '아이템 매칭 - 총 창고 아이템', '값': item_stats['total_warehouse_items']},
                        {'통계 항목': '아이템 매칭 - 매칭된 아이템', '값': item_stats['matched_items']},
                        {'통계 항목': '아이템 매칭 - 청구서 전용', '값': item_stats['invoice_only']},
                        {'통계 항목': '아이템 매칭 - 창고 전용', '값': item_stats['warehouse_only']},
                        {'통계 항목': '아이템 매칭 - 매칭률', '값': f"{item_stats['match_rate']:.1%}"}
                    ])
                ...
                if 'hvdccode_matching' in cross_validation['details']:
                    ...
                    cross_stats.extend([
                        {'통계 항목': 'HVDC CODE - 총 비교 항목', '값': hvdccode_stats['total_compared']},
                        {'통계 항목': 'HVDC CODE - 일치 항목', '값': hvdccode_stats['matches']},
                        {'통계 항목': 'HVDC CODE - 불일치 항목', '값': hvdccode_stats['mismatches']},
                        {'통계 항목': 'HVDC CODE - 일치률', '값': f"{hvdccode_stats['match_rate']:.1%}"}
                    ])
                ...
                if 'quantity_matching' in cross_validation['details']:
                    ...
                    cross_stats.extend([
                        {'통계 항목': '수량 매칭 - 총 비교 항목', '값': qty_stats['total_compared']},
                        {'통계 항목': '수량 매칭 - 일치 항목', '값': qty_stats['matches']},
                        {'통계 항목': '수량 매칭 - 불일치 항목', '값': qty_stats['mismatches']},
                        {'통계 항목': '수량 매칭 - 일치률', '값': f"{qty_stats['match_rate']:.1%}"}
                    ])
                
                # 데이터 완전성 통계
                if 'data_completeness' in cross_validation['details']:
                    completeness_stats = cross_validation['details']['data_completeness']
                    cross_stats.extend([
                        {'통계 항목': '데이터 완전성 - 청구서', '값': f"{completeness_stats['invoice_completeness']:.1%}"},
                        {'통계 항목': '데이터 완전성 - 창고', '값': f"{completeness_stats['warehouse_completeness']:.1%}"},
                        {'통계 항목': '데이터 완전성 - 평균', '값': f"{completeness_stats['average_completeness']:.1%}"}
                    ])
                ...
                if 'date_consistency' in cross_validation['details']:
                    ...
                    cross_stats.extend([
                        {'통계 항목': '날짜 일관성 - 청구서 범위', '값': f"{date_stats['invoice_date_range'][0]} ~ {date_stats['invoice_date_range'][1]}"},
                        {'통계 항목': '날짜 일관성 - 창고 범위', '값': f"{date_stats['warehouse_date_range'][0]} ~ {date_stats['warehouse_date_range'][1]}"},
                        {'통계 항목': '날짜 일관성 - 겹침 여부', '값': '예' if date_stats['overlap'] else '아니오'},
                        {'통계 항목': '날짜 일관성 - 겹침 범위', '값': f"{date_stats['overlap_range'][0]} ~ {date_stats['overlap_range'][1]}" if date_stats['overlap'] else '해당없음'}
                    ])
                
                pd.DataFrame(cross_stats).to_excel(writer, sheet_name='교차_검증_통계', index=False)
            
            # 6. HVDC CODE 불일치 상세
            if 'details' in cross_validation and 'hvdccode_matching' in cross_validation['details']:
                hvdccode_mismatches = cross_validation['details']['hvdccode_matching'].get('mismatch_details', [])
                if hvdccode_mismatches:
                    mismatch_df = pd.DataFrame(hvdccode_mismatches)
                    mismatch_df.to_excel(writer, sheet_name='HVDC_CODE_불일치', index=False)
                else:
                    # 불일치가 없는 경우 빈 시트 생성
                    pd.DataFrame({'메시지': ['HVDC CODE 불일치 항목이 없습니다.']}).to_excel(writer, sheet_name='HVDC_CODE_불일치', index=False)
            
            # 7. 수량 불일치 상세
            if 'details' in cross_validation and 'quantity_matching' in cross_validation['details']:
                qty_mismatches = cross_validation['details']['quantity_matching'].get('mismatch_details', [])
                if qty_mismatches:
                    qty_mismatch_df = pd.DataFrame(qty_mismatches)
                    qty_mismatch_df.to_excel(writer, sheet_name='수량_불일치', index=False)
                else:
                    # 불일치가 없는 경우 빈 시트 생성
                    pd.DataFrame({'메시지': ['수량 불일치 항목이 없습니다.']}).to_excel(writer, sheet_name='수량_불일치', index=False)
            
            # 8. 청구서 전용 아이템
            if 'details' in cross_validation and 'item_matching' in cross_validation['details']:
                invoice_only_count = cross_validation['details']['item_matching'].get('invoice_only', 0)
                if invoice_only_count > 0:
                    pd.DataFrame({'메시지': [f'청구서에만 존재하는 아이템: {invoice_only_count}건']}).to_excel(writer, sheet_name='청구서_전용_아이템', index=False)
                else:
                    pd.DataFrame({'메시지': ['청구서 전용 아이템이 없습니다.']}).to_excel(writer, sheet_name='청구서_전용_아이템', index=False)
            
            # 9. 창고 전용 아이템
            if 'details' in cross_validation and 'item_matching' in cross_validation['details']:
                warehouse_only_count = cross_validation['details']['item_matching'].get('warehouse_only', 0)
                if warehouse_only_count > 0:
                    pd.DataFrame({'메시지': [f'창고에만 존재하는 아이템: {warehouse_only_count}건']}).to_excel(writer, sheet_name='창고_전용_아이템', index=False)
                else:
                    pd.DataFrame({'메시지': ['창고 전용 아이템이 없습니다.']}).to_excel(writer, sheet_name='창고_전용_아이템', index=False)
            ...
            pd.DataFrame(compliance_details).to_excel(writer, sheet_name='규정_준수_검증', index=False)
            ...
            for validation_name, validation_data in [
                ...
                for error in validation_data.get('errors', []):
                    all_errors.append({'검증 유형': validation_name, '오류 내용': error})
                for warning in validation_data.get('warnings', []):
                    all_warnings.append({'검증 유형': validation_name, '경고 내용': warning})
            
            if all_errors:
                pd.DataFrame(all_errors).to_excel(writer, sheet_name='오류_목록', index=False)
            else:
                pd.DataFrame({'메시지': ['오류가 없습니다.']}).to_excel(writer, sheet_name='오류_목록', index=False)
            
            if all_warnings:
                pd.DataFrame(all_warnings).to_excel(writer, sheet_name='경고_목록', index=False)
            else:
                pd.DataFrame({'메시지': ['경고가 없습니다.']}).to_excel(writer, sheet_name='경고_목록', index=False)
        
        logger.info(f"✅ Excel 보고서 생성 완료: {excel_file}")
        return excel_file
    
    def _generate_rdf_report(self, timestamp: str, invoice_validation: Dict, warehouse_validation: Dict,
                           cross_validation: Dict, compliance_validation: Dict, overall_validation: Dict) -> Path:
        """RDF TTL 파일 생성"""
        ...
        g.add((validation_uri, self.LOGI.timestamp, Literal(datetime.now().isoformat())))
        g.add((validation_uri, self.LOGI.totalScore, Literal(overall_validation['total_score'], datatype=XSD.float)))
        ...
        for component, score in overall_validation['component_scores'].items():
            component_uri = URIRef(f"http://macho-gpt.com/validation/{timestamp}#{component}")
        ...
        for validation_type, validation_data in all_validations:
            for check_name, check_result in validation_data['checks'].items():
                check_uri = URIRef(f"http://macho-gpt.com/validation/{timestamp}#{validation_type}_{check_name}")
    ...
    def _generate_recommendations(self, overall_validation: Dict[str, Any]) -> List[str]:
        """권고사항 생성"""
        recommendations = []
        ...
        if overall_validation['component_scores']['invoice'] < 0.9:
            recommendations.append("청구서 데이터 검증 강화 필요 - 필수 컬럼 및 계산 정확도 확인")
        
        if overall_validation['component_scores']['warehouse'] < 0.9:
            recommendations.append("창고 데이터 검증 강화 필요 - 창고/현장 컬럼 구조 확인")
        
        if overall_validation['component_scores']['cross'] < 0.8:
            recommendations.append("교차 검증 개선 필요 - 아이템 매칭 및 수량 일치성 확인")
        
        if overall_validation['component_scores']['compliance'] < 0.8:
            recommendations.append("규정 준수 검증 강화 필요 - FANR/MOIAT 요구사항 확인")
...
# 명령어 처리 함수
def generate_validation_report(invoice_file: str, warehouse_file: str, config: ValidationConfig = None) -> Dict[str, Any]:
    """
...
if __name__ == '__main__':
    ...
    if not os.path.exists(test_invoice_file):
        ...
        print("="*80)
        print(f"전체 점수: {result['validation_results']['overall_validation']['total_score']:.3f}")
