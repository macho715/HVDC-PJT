...
class TestWarehouseFinalHandling:
    ...
    def test_final_handling_aggregate_by_location(self):
        ...
        for location in expected_locations:
            assert (
                location in final_handling.index
            ), f"{location} should be in final handling"
    ...
    def test_final_handling_with_real_data_pattern(self):
        ...
        test_data = pd.DataFrame(
            {
                ...
                "SHU": [None] * 696
                + [datetime(2024, 5, 20)] * 304,  # 1304 -> 304로 조정
        ...
        # Status별 집계
        status_summary = result_df.groupby("Status_Current")[
            "Status_Location"
        ].value_counts()
        print("\n📋 Status별 위치 분포:")
