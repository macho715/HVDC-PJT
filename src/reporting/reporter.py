...
class HVDCExcelReporter:
    ...
    def _generate_excel_file(self):
        ...
        self.excel_filename = (
            output_dir / f"HVDC_입고로직_종합리포트_{self.timestamp}.xlsx"
        )
    ...
    def _write_sheets(self, writer):
        ...
        # Data Sheets
        self.stats["processed_data"].head(1000).to_excel(
            writer, sheet_name="원본_데이터_샘플", index=False
        )
        self.stats["processed_data"][
            self.stats["processed_data"]["Vendor"] == "HITACHI"
        ].to_excel(writer, sheet_name="HITACHI_원본데이터", index=False)
        self.stats["processed_data"][
            self.stats["processed_data"]["Vendor"] == "SIMENSE"
        ].to_excel(writer, sheet_name="SIEMENS_원본데이터", index=False)
        self.stats["processed_data"].to_excel(
    ...
    def _create_warehouse_monthly_sheet(self) -> pd.DataFrame:
        ...
        for month_str in months:
            ...
            for wh in config.WAREHOUSE_COLUMNS:
                in_count = sum(
                    item["Pkg_Quantity"]
                    for item in self.stats["inbound_result"]["inbound_items"]
                    if item["Warehouse"] == wh and item["Year_Month"] == month_str
                )
                in_count += sum(
                    t["Pkg_Quantity"]
                    for t in self.stats["inbound_result"]["warehouse_transfers"]
                    if t["To_Warehouse"] == wh and t["Year_Month"] == month_str
                )
                ...
                out_count = sum(
                    item["Pkg_Quantity"]
                    for item in self.stats["outbound_result"]["outbound_items"]
                    if item["Warehouse"] == wh and item["Year_Month"] == month_str
                )
    ...
    def _create_site_monthly_sheet(self) -> pd.DataFrame:
        ...
        for month_str in months:
            ...
            for site in config.SITE_COLUMNS:
                mask = (
                    ...
                    & (
                        pd.to_datetime(df_processed[site]).dt.strftime("%Y-%m")
                        == month_str
                    )
                )
        ...
        if not df.empty:
            total_row = {
                f"입고_{s}": df[f"입고_{s}"].sum() for s in config.SITE_COLUMNS
            }
            total_row.update(
                {f"재고_{s}": df[f"재고_{s}"].iloc[-1] for s in config.SITE_COLUMNS}
            )
            total_row["입고월"] = "Total"
            df.loc[len(df)] = total_row
        return df
