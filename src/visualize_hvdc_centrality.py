...
# 노드 크기/색상 (중심성 기반)
node_sizes = [18 + 32 * (centrality[n]/max_central if max_central else 0) for n in g.nodes()]
node_colors = [f'rgba(255,{180+int(60*(centrality[n]/max_central if max_central else 0))},0,0.95)' for n in g.nodes()]
...
for u, v in g.edges():
    x0, y0, z0 = pos[u]
    x1, y1, z1 = pos[v]
    mx, my, mz = (x0+x1)/2 + np.random.uniform(-0.08,0.08), (y0+y1)/2 + np.random.uniform(-0.08,0.08), (z0+z1)/2 + np.random.uniform(-0.08,0.08)
...
for t in range(10):
    ...
    anim_colors = [f'rgba(255,{180+int(60*(centrality[n]/max_central if max_central else 0))},0,{0.7+0.3*np.sin(t+i):.2f})' for i, n in enumerate(g.nodes())]
...
fig = go.Figure(data=edge_traces + [glow_trace, node_trace], layout=layout, frames=frames)
fig.show() 