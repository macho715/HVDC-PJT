...
class CompletePivotAnalyzer:
    ...
    def load_existing_data(self):
        ...
        try:
            # 각 시트별로 데이터 로드
            self.df = {}
            self.df["inbound_detail"] = pd.read_excel(
                self.data_file, sheet_name="입고상세"
            )
            self.df["outbound_detail"] = pd.read_excel(
                self.data_file, sheet_name="출고상세"
            )
            self.df["original_data"] = pd.read_excel(
                self.data_file, sheet_name="원본데이터"
            )
    ...
    def create_monthly_outbound_pivot(self):
        ...
        # 출고 날짜를 월별로 변환
        outbound_df["Outbound_Month"] = pd.to_datetime(
            outbound_df["Outbound_Date"]
        ).dt.to_period("M")
    ...
    def create_site_inbound_pivot(self):
        ...
        # 2. 현장별 총 입고 집계
        site_total_pivot = (
            site_inbound_df.groupby("Site")["PKG_Quantity"]
            .sum()
            .to_frame("Total_Inbound")
        )
    ...
    def run_complete_analysis(self):
        ...
        self.analysis_results["delivery_transfer"] = (
            self.analyze_direct_delivery_and_transfer()
        )
    ...
    def export_complete_results(self):
        ...
        try:
            with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
                ...
                if "outbound_pivot" in self.analysis_results:
                    outbound = self.analysis_results["outbound_pivot"]
                    outbound["warehouse_monthly"].to_excel(
                        writer, sheet_name="월별창고출고피벗"
                    )
                    outbound["site_monthly"].to_excel(
                        writer, sheet_name="월별현장출고피벗"
                    )
                    outbound["warehouse_site_matrix"].to_excel(
                        writer, sheet_name="창고현장출고매트릭스"
                    )

                # 2. 현장 입고 피벗
                if (
                    "site_inbound_pivot" in self.analysis_results
                    and self.analysis_results["site_inbound_pivot"]
                ):
                    site_inbound = self.analysis_results["site_inbound_pivot"]
                    site_inbound["site_monthly"].to_excel(
                        writer, sheet_name="월별현장입고피벗"
                    )
                    site_inbound["site_total"].to_excel(
                        writer, sheet_name="현장별총입고"
                    )
                    site_inbound["site_inbound_data"].to_excel(
                        writer, sheet_name="현장입고상세", index=False
                    )
                ...
                if "delivery_transfer" in self.analysis_results:
                    ...
                    if (
                        "direct_monthly_pivot" in delivery
                        and not delivery["direct_monthly_pivot"].empty
                    ):
                        delivery["direct_monthly_pivot"].to_excel(
                            writer, sheet_name="월별직배송피벗"
                        )

                    if (
                        "warehouse_transfer_monthly" in delivery
                        and not delivery["warehouse_transfer_monthly"].empty
                    ):
                        delivery["warehouse_transfer_monthly"].to_excel(
                            writer, sheet_name="월별창고이동피벗"
                        )
                        delivery["warehouse_transfer_matrix"].to_excel(
                            writer, sheet_name="창고이동매트릭스"
                        )
                    ...
                    if delivery["warehouse_transfer_data"]:
                        ...
                        transfer_df.to_excel(
                            writer, sheet_name="창고이동상세", index=False
                        )
