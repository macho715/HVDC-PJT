...
class CompleteHVDCAnalyzer:
    ...
    def load_complete_data(self):
        ...
        try:
            ...
            print(f"📅 데이터 기간: {self.df.iloc[:, 1].min()} ~ {self.df.iloc[:, 1].max()}")
    ...
    def basic_warehouse_analysis(self):
        ...
        for col in self.df.columns:
            ...
            elif any(site_word in col_lower for site_word in ['site', '현장', 'project']):
    ...
    def generate_monthly_analysis(self):
        ...
        try:
            # 날짜 컬럼 찾기
            date_cols = [col for col in self.df.columns if 'date' in col.lower() or 'arrival' in col.lower()]
        ...
        try:
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                ...
                if 'monthly_data' in locals():
                    monthly_data.to_frame('건수').to_excel(writer, sheet_name='월별분석')
    ...
    def print_summary(self):
        ...
        if 'total_records' in self.analysis_results:
            ...
            print(f"📅 날짜 데이터 포함: {self.analysis_results['records_with_dates']:,}건")
