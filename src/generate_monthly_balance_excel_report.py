...
class MonthlyBalanceExcelReporter:
    ...
    def generate_raw_data_sheet(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df_with_final.iterrows():
            ...
            if warehouse_dates:
                ...
                total_warehouse_days = max(
                    0, (max(warehouse_dates) - min(warehouse_dates)).days
                )
        ...
        for month in sorted(all_months):
            for warehouse in warehouses:
                ...
                if monthly_outbound_events.get(month, 0) > 0 and inbound_count > 0:
                    total_monthly_inbound = sum(
                        monthly_inbound.get("by_warehouse", {}).values()
                    )
                    if total_monthly_inbound > 0:
                        outbound_ratio = inbound_count / total_monthly_inbound
                        outbound_count = round(
                            monthly_outbound_events.get(month, 0) * outbound_ratio
                        )
                ...
                if monthly_warehouse_transfer.get(month, 0) > 0:
                    transfer_count = round(
                        monthly_warehouse_transfer.get(month, 0) / len(warehouses)
                    )
                ...
                warehouse_data.append(
                    {
                        ...
                        "Utilization": round(
                            (inbound_count / max(1, inbound_count + inventory_count))
                            * 100,
                            1,
                        ),
                    }
    ...
    def generate_site_monthly_sheet(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for month in sorted(monthly_site_inbound.keys()):
            for site in sites:
                ...
                for _, row in df.iterrows():
                    if (
                        row.get("Status_Current") == "site"
                        and row.get("Status_Location") == site
                    ):
                        site_inventory += 1
                ...
                if site_inbound_count > 0:
                    consumption_rate = round(
                        (
                            site_inbound_count
                            / max(1, site_inventory + site_inbound_count)
                        )
                        * 100,
                        1,
                    )
                ...
                site_data.append(
                    {
                        ...
                        "Delivery_Efficiency": round(
                            (
                                site_inbound_count
                                / max(1, site_inbound_count + direct_count)
                            )
                            * 100,
                            1,
                        ),
    ...
    def generate_excel_report(self, output_path: str = None) -> str:
        ...
        warehouse_monthly_formatted = self.create_multi_level_headers(
            warehouse_monthly.copy(), "warehouse"
        )

        print("🏗️ 시트 3: 현장 월별 입고재고 생성 중...")
        site_monthly = self.generate_site_monthly_sheet(df)
        site_monthly_formatted = self.create_multi_level_headers(
            site_monthly.copy(), "site"
        )
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            ...
            test_results = pd.DataFrame(
                [
                    ...
                    {
                        "Test_Name": "test_fail_safe_mode_trigger",
                        "Status": "✅ PASSED",
                        "Description": "Fail-safe 모드 권장",
                    },
                ]
            )
        ...
        if summary_stats["fail_safe_recommendation"]["switch_to_zero"]:
            ...
            print(
                f"✅ 권장사항: {summary_stats['fail_safe_recommendation']['recommended_action']}"
            )
