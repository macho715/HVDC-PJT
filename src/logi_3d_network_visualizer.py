    ...
    def load_logistics_data(self, data_source: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                'next_cmds': ['/visualize_3d_network', '/analyze_flow_patterns', '/generate_kpi_dashboard']
            }
            
            self.logger.info(f"Logistics data loaded: {result['nodes_count']} nodes, {result['edges_count']} edges")
            return result
    ...
    def _build_network_graph(self, df: pd.DataFrame) -> None:
        ...
        for warehouse in self.hvdc_warehouses:
            if warehouse in df.columns:
                self.G.add_node(warehouse, type=NetworkNodeType.WAREHOUSE, 
                               category='warehouse', capacity=1000)
        
        for site in self.hvdc_sites:
            if site in df.columns:
                self.G.add_node(site, type=NetworkNodeType.SITE, 
                               category='site', capacity=500)
        ...
        for node, node_type in additional_nodes.items():
            self.G.add_node(node, type=node_type, category=node_type.value, capacity=2000)
        ...
        for warehouse in self.hvdc_warehouses:
            for site in self.hvdc_sites:
                if warehouse in df.columns and site in df.columns:
                    # 실제 데이터 기반 연결 강도 계산
                    connection_strength = self._calculate_connection_strength(df, warehouse, site)
                    if connection_strength > 0:
                        self.G.add_edge(warehouse, site, 
        ...
        for supplier in suppliers:
            for warehouse in self.hvdc_warehouses:
                if supplier in self.G.nodes() and warehouse in self.G.nodes():
                    self.G.add_edge(supplier, warehouse,
                                  weight=0.8,
        ...
        for port in ports:
            for warehouse in self.hvdc_warehouses:
                if port in self.G.nodes() and warehouse in self.G.nodes():
                    self.G.add_edge(port, warehouse,
                                  weight=0.9,
                                  type='port_to_warehouse',
                                  distance=30)
    
    def _calculate_connection_strength(self, df: pd.DataFrame, source: str, target: str) -> float:
        """두 노드 간 연결 강도 계산"""
        try:
            # 실제 데이터에서 연결 패턴 분석
            ...
            # 평균 시간 차이 계산
            avg_time_diff = abs((source_dates.mean() - target_dates.mean()).total_seconds() / 86400)
    ...
    def create_logistics_dashboard(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig = make_subplots(
                ...
                subplot_titles=('3D Network', 'Warehouse Performance', 
                              'Flow Timeline', 'Node Distribution'),
    ...
    def _create_flow_timeline(self, df: pd.DataFrame) -> Dict[str, List]:
        ...
        if not date_columns:
            ...
            if all_dates:
                ...
                monthly_flows = date_series.dt.to_period('M').value_counts().sort_index()
...
def main():
    ...
    sample_data = pd.DataFrame({
        ...
        'DSV Indoor': [datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(100)],
        'DSV Outdoor': [datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(100)],
        'MIR': [datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(100)],
        'SHU': [datetime.now() - timedelta(days=np.random.randint(1, 365)) for _ in range(100)],
    ...
    if load_result['status'] == 'SUCCESS':
        print(f"✅ 데이터 로드 완료: {load_result['nodes_count']} 노드, {load_result['edges_count']} 엣지")
        ...
        # 대시보드도 내보내기
        dashboard_result = visualizer.export_visualization(fig_dashboard, "HVDC_Dashboard_Demo")
