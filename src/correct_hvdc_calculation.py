...
class CorrectHVDCCalculator:
    ...
    def load_real_data(self):
        ...
        try:
            ...
            # 실제 창고 컬럼 확인
            actual_warehouse_cols = [col for col in self.warehouse_columns if col in self.df.columns]
            print(f"🏭 발견된 창고 컬럼: {actual_warehouse_cols}")
            
            # 실제 현장 컬럼 확인
            actual_site_cols = [col for col in self.site_columns if col in self.df.columns]
    ...
    def calculate_correct_outbound(self):
        ...
        # 실제 존재하는 컬럼만 필터링
        existing_warehouse_cols = [col for col in self.warehouse_columns if col in self.df.columns]
        existing_site_cols = [col for col in self.site_columns if col in self.df.columns]
    ...
    def run_correct_analysis(self):
        ...
        self.analysis_results['direct_delivery'] = self.calculate_correct_direct_delivery()
        ...
        try:
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                ...
                for key, value in self.analysis_results.items():
                    ...
                    if isinstance(value, dict):
                        for sub_key, sub_value in value.items():
                            if sub_key not in ['inbound_items', 'site_items', 'direct_items', 'warehouse_items', 'outbound_events']:
                                summary_data.append({
                                    '분석항목': f"{key}_{sub_key}",
                                    '값': sub_value
                                })
                    else:
                        summary_data.append({
                ...
                if 'monthly_pivot' in self.analysis_results:
                    self.analysis_results['monthly_pivot'].to_excel(writer, sheet_name='월별입고피벗')
                
                # 입고 상세 데이터
                if 'inbound' in self.analysis_results and 'inbound_items' in self.analysis_results['inbound']:
                    inbound_df = pd.DataFrame(self.analysis_results['inbound']['inbound_items'])
                    inbound_df.to_excel(writer, sheet_name='입고상세', index=False)
                
                # 출고 상세 데이터
                if 'outbound' in self.analysis_results and 'outbound_events' in self.analysis_results['outbound']:
                    outbound_df = pd.DataFrame(self.analysis_results['outbound']['outbound_events'])
