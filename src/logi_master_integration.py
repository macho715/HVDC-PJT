...
class LogiMasterIntegration:
    ...
    async def execute_task_manager_command(
    ...
    async def execute_macho_gpt_command(
        self, command: str, args: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """MACHO-GPT 명령어 실행"""
        if not await self.initialize():
    ...
    async def _create_task(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"status": "SUCCESS", "tasks_created": len(default_tasks)}
    ...
    async def _macho_switch_mode(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"status": "SUCCESS", "mode": mode, "new_tasks_created": len(new_tasks)}
    ...
    async def _macho_weather_check(self, args: Dict[str, Any]) -> Dict[str, Any]:
        """MACHO-GPT 날씨 확인"""
        print(
            f"🌤️ MACHO-GPT의 실시간 날씨 영향 분석 및 ETA 예측이 성공적으로 실행되었습니다."
        )
...
async def main():
    ...
    category = command_parts[0] + (
        "_" + command_parts[1] if len(command_parts) > 2 else ""
    )
    ...
    try:
        if category == "task_manager":
            result = await integration.execute_task_manager_command(
