...
class HVDCExcelRecreator:
    ...
    def _calculate_flow_codes(self):
        ...
        if 'wh handling' in self.df_combined.columns:
            ...
            self.df_combined['FLOW_CODE'] = self.df_combined['wh handling'].fillna(0).astype(int)
            ...
            for idx, row in self.df_combined.iterrows():
                warehouse_count = 0
                for col in self.warehouse_columns.values():
                    if col.lower() in self.df_combined.columns and pd.notna(row[col.lower()]):
                        warehouse_count += 1
    ...
    def create_sheet1_transaction_data(self) -> pd.DataFrame:
        ...
        # 사용 가능한 컬럼만 선택
        available_columns = [col for col in core_columns if col in self.df_combined.columns]
        
        sheet1_data = self.df_combined[available_columns].copy()
        
        logger.info(f"✅ Sheet 1 생성 완료: {len(sheet1_data)}건, {len(available_columns)}개 컬럼")
        return sheet1_data
    ...
    def create_sheet2_flowcode_analysis(self) -> pd.DataFrame:
        ...
        # Flow Code별 분포 계산
        flowcode_stats = self.df_combined.groupby('FLOW_CODE').agg({
            'VENDOR': 'count'
        }).rename(columns={'VENDOR': 'Count'})
        
        # 백분율 계산
        total_count = len(self.df_combined)
        flowcode_stats['Percentage'] = (flowcode_stats['Count'] / total_count * 100).round(2)
    ...
    def create_sheet4_warehouse_multilevel(self) -> pd.DataFrame:
        ...
        logger.info(f"✅ Sheet 4 생성 완료: {len(result)}개월 × {len(self.warehouse_columns)}개 창고")
        return result
    ...
    def create_sheet5_site_multilevel(self) -> pd.DataFrame:
        ...
        logger.info(f"✅ Sheet 5 생성 완료: {len(result)}개월 × {len(self.site_columns)}개 현장")
        return result
    
    def _calculate_warehouse_inbound(self, warehouse_col: str, period: pd.Timestamp) -> int:
        """창고별 월별 입고 계산 (REV.MD calculate_warehouse_inbound_correct 기반)"""
        col_name = warehouse_col if warehouse_col in self.df_combined.columns else warehouse_col.lower()
    ...
    def _calculate_warehouse_outbound(self, warehouse_col: str, period: pd.Timestamp) -> int:
        ...
        for _, row in warehouse_visited.iterrows():
            ...
            for other_col in self.warehouse_columns.values():
                other_col_name = other_col if other_col in self.df_combined.columns else other_col.lower()
                if other_col_name != col_name and other_col_name in self.df_combined.columns:
            ...
            for site_col in self.site_columns.values():
                site_col_name = site_col if site_col in self.df_combined.columns else site_col.lower()
    ...
    def create_excel_report(self) -> str:
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            sheet1_data.to_excel(writer, sheet_name='전체_트랜잭션_FLOWCODE0-4', index=False)
            ...
            if not sheet4_data.empty and 'Message' not in sheet4_data.columns:
                ...
                sheet4_data.to_excel(writer, sheet_name='창고별_월별_입출고_완전체계', index=False)
            ...
            if not sheet5_data.empty and 'Message' not in sheet5_data.columns:
                ...
                sheet5_data.to_excel(writer, sheet_name='현장별_월별_입고재고_완전체계', index=False)
