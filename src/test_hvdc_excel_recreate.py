...
class TestHVDCExcelRecreate:
    ...
    def test_data_loading_and_record_counts(self):
        ...
        assert len(df_combined) == self.expected_total_records, \
        ...
        assert hitachi_count >= self.expected_hitachi_records - 100, \
            f"HITACHI records should be approximately {self.expected_hitachi_records}, got {hitachi_count}"
        assert simense_count >= self.expected_simense_records - 100, \
    ...
    def test_flow_code_calculation(self):
        ...
        assert df_combined['FLOW_CODE'].between(0, 4).all(), \
    ...
    def test_multi_level_headers(self):
        ...
        try:
            warehouse_df = pd.read_excel(excel_path, 
                                       sheet_name='창고별_월별_입출고_완전체계',
                                       header=[0, 1])
            
            assert warehouse_df.columns.nlevels == 2, \
        ...
        try:
            site_df = pd.read_excel(excel_path,
                                   sheet_name='현장별_월별_입고재고_완전체계', 
                                   header=[0, 1])
            
            assert site_df.columns.nlevels == 2, \
    ...
    def test_flow_code_analysis_summary(self):
        ...
        flow_analysis_df = pd.read_excel(excel_path, 
                                        sheet_name='FLOWCODE0-4_분석요약')
        
        # Should have 5 rows (Flow Code 0-4)
        assert len(flow_analysis_df) == 5, \
        ...
        for col in required_columns:
            assert col in flow_analysis_df.columns, \
    ...
    def test_pre_arrival_analysis(self):
        ...
        pre_arrival_df = pd.read_excel(excel_path, 
                                      sheet_name='Pre_Arrival_상세분석')
        
        # Should contain only Flow Code 0 records (if FLOW_CODE column exists)
        if 'FLOW_CODE' in pre_arrival_df.columns:
            assert (pre_arrival_df['FLOW_CODE'] == 0).all(), \
                "Pre-Arrival sheet should contain only Flow Code 0 records"
