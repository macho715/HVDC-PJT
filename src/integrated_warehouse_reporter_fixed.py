...
class IntegratedWarehouseReporterFixed:
    ...
    def load_warehouse_data(self) -> pd.DataFrame:
        ...
        if data_frames:
            ...
            # 날짜 컬럼 처리
            date_columns = ["ETA/ATA", "ETD/ATD"] + list(
                self.warehouse_columns.values()
            )
    ...
    def calculate_warehouse_inbound_correct(
        self, df: pd.DataFrame, warehouse_name: str, period: pd.Period
    ) -> int:
    ...
    def calculate_warehouse_outbound_correct(
        self, df: pd.DataFrame, warehouse_name: str, period: pd.Period
    ) -> int:
    ...
    def calculate_stock_levels(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in periods:
            for warehouse_name in self.warehouse_info.keys():
                # 입고 계산
                inbound = self.calculate_warehouse_inbound_correct(
                    df, warehouse_name, period
                )
                # 출고 계산
                outbound = self.calculate_warehouse_outbound_correct(
                    df, warehouse_name, period
                )
                # 재고 계산
    ...
    def calculate_site_inventory_correct(
        self, df: pd.DataFrame, period: pd.Period
    ) -> pd.DataFrame:
        ...
        for site in df["Site"].dropna().unique():
            ...
            total_weight = (
                site_df["N.W(kgs)"].sum() if "N.W(kgs)" in site_df.columns else 0
            )
    ...
    def analyze_warehouse_inbound_logic(self) -> Dict:
        ...
        analysis_result = {
            ...
            "direct_transport_ratio": (
                flow_distribution.get(0, 0) / total_items if total_items > 0 else 0
            ),
            "warehouse_utilization": {
                wh: visits / total_items
                for wh, visits in warehouse_visits.items()
                if total_items > 0
            },
        }
    ...
    def generate_comprehensive_report(self) -> str:
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            ...
            if not stock_levels.empty:
                stock_levels.to_excel(
                    writer, sheet_name="01_창고별재고현황", index=False
                )
            ...
            if inbound_analysis:
                ...
                for key, value in inbound_analysis.items():
                    if isinstance(value, dict):
                        ...
                        analysis_rows.append(
                            {"Category": "Summary", "Item": key, "Value": value}
                        )
